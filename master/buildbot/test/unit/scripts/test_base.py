...
class TestSubcommandOptions(unittest.TestCase):
    def fakeOptionsFile(self, **kwargs):
        self.patch(
            base.SubcommandOptions, "loadOptionsFile", lambda self: kwargs.copy()
        )
...
class TestLoadOptionsFile(
    dirs.DirsMixin, misc.StdoutAssertionsMixin, unittest.TestCase
):
    def setUp(self):
    ...
    def do_loadOptionsFile(self, _here, exp):
        ...
        if runtime.platformType == "win32":
            ...
            patches.append(
                self.patch(shell, "SHGetFolderPath", lambda *args: self.home)
            )
        else:
