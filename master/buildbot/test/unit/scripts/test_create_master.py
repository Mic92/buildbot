...
class TestCreateMaster(misc.StdoutAssertionsMixin, unittest.TestCase):
    ...
    def do_test_createMaster(self, config):
        ...
        repls["createDB"].side_effect = lambda config: calls.append(
            fn
        ) or defer.succeed(None)
...
class TestCreateMasterFunctions(
    ...
    def assertDBSetup(
        self, basedir=None, db_url="sqlite:///state.sqlite", verbose=True
    ):
    ...
    def test_makeSampleConfig_db(self):
        create_master.makeSampleConfig(
            mkconfig(basedir="test", db="XXYYZZ", quiet=True)
        )
        with open(
            os.path.join("test", "master.cfg.sample"), "rt", encoding="utf-8"
        ) as f:
