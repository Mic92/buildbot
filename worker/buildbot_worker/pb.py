    ...
    def protocol_args_setup(self, command, args):
        ...
        if command == "stat":
            args["path"] = os.path.join(
                self.basedir, args.get("workdir", ""), args["file"]
            )
            del args["file"]
...
class WorkerForBuilderPbLike(WorkerForBuilderBase):
    ...
    def setBuilddir(self, builddir):
        ...
        self.basedir = os.path.join(
            bytes2unicode(self.bot.basedir), bytes2unicode(self.builddir)
        )
    ...
    def stopCommand(self):
        ...
        if not self.protocol_command:
            return
        log.msg(
            "stopCommand: halting current command {0}".format(
                ...
                if dir not in wanted_dirs:
                    if self.delete_leftover_dirs:
                        ...
                        try:
                            ...
                            log.msg(
                                "Cannot remove directory '{0}': " "{1}".format(dir, e)
                            )
                    else:
                        log.msg(
...
class BotFactory(AutoLoginPBFactory):
    ...
    def __init__(
        self, buildmaster_host, port, keepaliveInterval, maxDelay, retryPolicy=None
    ):
    ...
    def startTimers(self):
        ...
        self.keepaliveTimer = self._reactor.callLater(
            self.keepaliveInterval, doKeepalive
        )
...
class Worker(WorkerBase):
    ...
    def startService(self):
        ...
        if self.allow_shutdown == "signal":
            ...
            log.msg(
                "Watching {0}'s mtime to initiate shutdown".format(self.shutdown_file)
            )
    ...
    def _checkShutdownFile(self):
        if (
            ...
            log.msg(
                "Initiating shutdown because {0} was touched".format(self.shutdown_file)
            )
