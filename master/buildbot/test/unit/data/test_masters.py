...
class MasterEndpoint(endpoint.EndpointMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db.insert_test_data(
            [
                fakedb.Master(
                    id=13, name="some:master", active=False, last_active=SOMETIME
                ),
                fakedb.Master(
                    id=14, name="other:master", active=False, last_active=SOMETIME
                ),
    ...
    def setUp(self):
        ...
        self.db.insert_test_data(
            [
                ...
                fakedb.Master(
                    id=14, name="other:master", active=True, last_active=OTHERTIME
                ),
...
class Master(TestReactorMixin, interfaces.InterfaceTests, unittest.TestCase):
    def setUp(self):
        ...
        self.master = fakemaster.make_master(
            self, wantMq=True, wantDb=True, wantData=True
        )
    ...
    def test_masterStopped(self):
        ...
        self.master.db.insert_test_data(
            [
                fakedb.Master(
                    id=13, name="aname", active=1, last_active=self.reactor.seconds()
                ),
            ]
        )
    ...
    def test_masterDeactivated(self):
        self.master.db.insert_test_data(
            [
                ...
                fakedb.LogChunk(
                    logid=2000, first_line=1, last_line=2, content="ab\ncd"
                ),
            ]
        )
        ...
        for rtype in "builder", "scheduler", "changesource":
            ...
            m = mock.Mock(
                name=f"{rtype}._masterDeactivated", spec=rtype_obj._masterDeactivated
            )
        ...
        self.master.data.rtypes.builder._masterDeactivated.assert_called_with(
            masterid=14
        )
        self.master.data.rtypes.scheduler._masterDeactivated.assert_called_with(
            masterid=14
        )
        self.master.data.rtypes.changesource._masterDeactivated.assert_called_with(
            masterid=14
        )
