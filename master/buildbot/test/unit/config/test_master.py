...
class ConfigLoaderTests(
    ConfigErrorsMixin, dirs.DirsMixin, unittest.SynchronousTestCase
):
    def setUp(self):
    ...
    def install_config_file(self, config_file, other_files=None):
        ...
        with open(
            os.path.join(self.basedir, self.filename), "w", encoding="utf-8"
        ) as f:
            f.write(config_file)
...
class MasterConfig_loaders(ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_load_global_logCompressionMethod(self):
        self.do_test_load_global(
            {"logCompressionMethod": "bz2"}, logCompressionMethod="bz2"
        )
    ...
    def test_load_global_collapseRequests_callable(self):
        ...
        self.do_test_load_global(
            {"collapseRequests": callable}, collapseRequests=callable
        )
    ...
    def test_load_global_prioritizeBuilders_callable(self):
        callable = lambda: None
        self.do_test_load_global(
            {"prioritizeBuilders": callable}, prioritizeBuilders=callable
        )
    ...
    def test_load_global_select_next_worker_callable(self):
        callable = lambda: None
        self.do_test_load_global(
            {"select_next_worker": callable}, select_next_worker=callable
        )
    ...
    def test_load_caches_buildCacheSize_and_caches(self):
        with capture_config_errors() as errors:
            self.cfg.load_caches(
                self.filename, {"buildCacheSize": 13, "caches": {"builds": 11}}
            )
    ...
    def test_load_caches_changeCacheSize_and_caches(self):
        with capture_config_errors() as errors:
            self.cfg.load_caches(
                self.filename, {"changeCacheSize": 13, "caches": {"changes": 11}}
            )
    ...
    def test_load_caches_to_small_err(self):
        ...
        self.assertConfigError(
            errors, "'Changes' cache size must be at least 1, got '-12'"
        )
    ...
    def test_load_builders(self):
        bldr = config.BuilderConfig(
            name="x", factory=factory.BuildFactory(), workername="x"
        )
    ...
    def test_load_workers_reserved_names(self, worker_name):
        with capture_config_errors() as errors:
            self.cfg.load_workers(
                self.filename, {"workers": [worker.Worker(worker_name, "x")]}
            )
    ...
    def test_load_workers_too_long(self):
        with capture_config_errors() as errors:
            name = "a" * 51
            self.cfg.load_workers(
                self.filename, {"workers": [worker.Worker(name, "x")]}
            )
    ...
    def test_load_change_sources_not_instance(self):
        with capture_config_errors() as errors:
            self.cfg.load_change_sources(
                self.filename, {"change_source": [mock.Mock()]}
            )
    ...
    def test_load_www_plugin(self):
        self.cfg.load_www(
            self.filename, {"www": {"plugins": {"waterfall": {"foo": "bar"}}}}
        )
        self.assertResults(
            www={
                "port": None,
...
class MasterConfig_checkers(ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_check_builders_unknown_worker(self):
        with capture_config_errors() as errors:
            ...
            b1 = FakeBuilder(
                workernames=["xyz", "abc"], builddir="x", name="b1", project=None
            )
    ...
    def test_check_builders_unknown_project(self):
        with capture_config_errors() as errors:
            ...
            b1 = FakeBuilder(
                workernames=[], builddir="x", name="b1", project="proj_unknown"
            )
            self.cfg.builders = [b1]

            self.cfg.check_builders()
        self.assertConfigError(
            errors, "builder 'b1' uses unknown project name 'proj_unknown'"
        )
    ...
    def test_check_builders(self):
        with capture_config_errors() as errors:
            ...
            b1 = FakeBuilder(
                workernames=["a"], name="b1", builddir="dir1", project="proj1"
            )
            b2 = FakeBuilder(
                workernames=["a"], name="b2", builddir="dir2", project=None
            )
            self.cfg.builders = [b1, b2]
