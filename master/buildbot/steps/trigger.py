...
class Trigger(BuildStep):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not hasRenderable(schedulerNames) and not hasRenderable(
            unimportantSchedulerNames
        ):
    ...
    def prepareSourcestampListForTrigger(self):
        if self.sourceStamps:
            ...
            for ss in self.sourceStamps:
                ...
                assert (
                    codebase not in ss_for_trigger
                ), "codebase specified multiple times"
    ...
    def run(self):
        ...
        if self.waitForFinish:
            ...
            for d in dl:
                d.addErrback(
                    log.err, "(ignored) while invoking Triggerable schedulers:"
                )
    ...
    def getCurrentSummary(self):
        ...
        if self._result_list:
            for status in ALL_RESULTS:
                ...
                if count:
                    summary = summary + (
                        f", {self._result_list.count(status)} "
