...
class ContactMixin(TestReactorMixin):
    ...
    def setUp(self):
        ...
        self.master = fakemaster.make_master(
            self, wantMq=True, wantData=True, wantDb=True
        )
        ...
        self.contact = self.contactClass(
            user=self.USER, channel=self.bot.getChannel(self.CHANNEL)
        )
...
class TestContact(ContactMixin, unittest.TestCase):
    ...
    def test_command_notify0(self):
        ...
        yield self.assertFailure(
            self.do_test_command("notify", args="off finished"), KeyError
        )
    ...
    def test_notify_missing_worker(self):
        ...
        self.assertEquals(
            self.sent[1], "Worker `work` is missing. It was seen last on sometime."
        )
    ...
    def test_command_stop_builder0_no_builds(self):
        yield self.do_test_command(
            "stop", args=f"build {self.BUILDER_NAMES[0]} 'i have a reason'"
        )
        self.assertEqual(len(self.sent), 1)
