...
class TestGitLab(
    ...
    def setup_step(self, step, args, **kwargs):
        ...
        step.build.properties.setProperty(
            "source_branch", "ms-viewport", "gitlab source branch"
        )
        step.build.properties.setProperty(
            "source_git_ssh_url",
        ...
        step.build.properties.setProperty(
            "source_project_id", 2337, "gitlab source project ID"
        )
        step.build.properties.setProperty(
            "target_branch", "master", "gitlab target branch"
        )
        step.build.properties.setProperty(
            "target_git_ssh_url",
        ...
        step.build.properties.setProperty(
            "target_project_id", 239, "gitlab target project ID"
        )
        return step
    ...
    def test_with_merge_branch(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "checkout", "-B", "ms-viewport"]
            ).exit(0),
