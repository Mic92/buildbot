...
class RunFakeMasterTestCase(
    unittest.TestCase, TestReactorMixin, DebugIntegrationLogsMixin
):
    def setUp(self):
    ...
    def do_test_build(self, builder_id):
        ...
        consumer = yield self.master.mq.startConsuming(
            on_finished, ("builds", None, "finished")
        )
...
class RunMasterBase(unittest.TestCase):
    ...
    def setup_master(self, config_dict, startWorker=True, **worker_kwargs):
        ...
        if startWorker:
            ...
            config_dict["workers"] = [
                workerclass(
                    "local1", password=Interpolate("localpw"), missing_timeout=0
                )
            ]
        ...
        self.master = m
        self.assertFalse(
            stop.called, "startService tried to stop the reactor; check logs"
        )
    ...
    def doForceBuild(
        ...
        newConsumer = yield self.master.mq.startConsuming(
            newCallback, ("buildsets", None, "new")
        )
        ...
        if triggerCallback is not None:
            ...
            yield self.master.data.control(
                "force", forceParams, ("forceschedulers", "force")
            )
        else:
        ...
        builds = yield self.master.data.get(
            ...
            filters=[
                resultspec.Filter(
                    "buildrequestid", "eq", [buildrequest["buildrequestid"]]
                )
            ],
        )
    ...
    def printBuild(self, build, out=sys.stdout, withLogs=False):
        ...
        yield self.enrichBuild(
            build, wantSteps=True, wantProperties=True, wantLogs=True
        )
        print(
            ...
            for log in step["logs"]:
                for line in log["contents"]["content"].splitlines():
                    if onlyStdout and line[0] != "o":
                        continue
                    expectedLog = self._match_patterns_consume(
                        line, expectedLog, is_regex=regex
                    )
        if expectedLog:
