...
class IRCContact(Contact):
    ...
    def op_required(self, command):
        if self.is_private_chat or self.user_id in self.bot.authz.get(
            command.upper(), ()
        ):
            return False
    ...
    command_HUSTLE.usage = (
        "dondon on #qutebrowser: qutebrowser-bb needs to learn to do the hustle"
    )
...
class IrcStatusBot(StatusBot, irc.IRCClient):
    ...
    def privmsg(self, user, channel, message):
        ...
        if message.startswith(f"{self.nickname}:") or message.startswith(
            f"{self.nickname},"
        ):
