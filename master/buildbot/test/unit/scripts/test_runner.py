...
class TestCreateMasterOptions(OptionsMixin, unittest.TestCase):
    ...
    def test_db_invalid(self):
        with self.assertRaisesRegex(
            usage.UsageError, "could not parse database URL 'inv_db_url'"
        ):
...
class TestSendChangeOptions(OptionsMixin, unittest.TestCase):
    ...
    def test_properties(self):
        opts = self.parse(
            "--property", "x:y", "--property", "a:b", *self.master_and_who
        )
    ...
    def test_comments_overrides_logfile(self):
        opts = self.parse(
            "--logfile", "logs", "--comments", "foo", *self.master_and_who
        )
...
class TestUserOptions(OptionsMixin, unittest.TestCase):
    ...
    def test_info_with_id(self):
        opts = self.parse(
            "--info", "tdurden:svn=marla", "--op", "update", *self.extra_args
        )
...
class TestRun(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patch(
            runner.Options, "subCommands", [["my", None, self.MySubCommand, "my, my"]]
        )
