...
class FakeUpdates(service.AsyncService):
    ...
    def addBuildset(
        ...
        for ss in sourcestamps:
            if not isinstance(ss, int) and not isinstance(ss, dict):
                self.testcase.fail(
                    f"{ss} ({type(ss)}) is not an integer or a dictionary"
                )
    ...
    def completeBuildRequests(self, brids, results, complete_at=None):
        ...
        validation.verifyType(
            self.testcase, "results", results, validation.IntValidator()
        )
        validation.verifyType(
            self.testcase,
            "complete_at",
    ...
    def find_project_id(self, name):
        validation.verifyType(
            self.testcase, "project name", name, validation.StringValidator()
        )
    ...
    def forget_about_it(self, name):
        validation.verifyType(
            self.testcase, "scheduler name", name, validation.StringValidator()
        )
    ...
    def findChangeSourceId(self, name):
        ...
        if name not in self.changesourceIds:
            self.changesourceIds[name] = (
                max([0] + list(self.changesourceIds.values())) + 1
            )
        return defer.succeed(self.changesourceIds[name])

    def findBuilderId(self, name):
        validation.verifyType(
            self.testcase, "builder name", name, validation.StringValidator()
        )
    ...
    def addBuild(self, builderid, buildrequestid, workerid):
        validation.verifyType(
            self.testcase, "builderid", builderid, validation.IntValidator()
        )
        validation.verifyType(
            self.testcase, "buildrequestid", buildrequestid, validation.IntValidator()
        )
        validation.verifyType(
            self.testcase, "workerid", workerid, validation.IntValidator()
        )
        return defer.succeed((10, 1))

    def generateNewBuildEvent(self, buildid):
        validation.verifyType(
            self.testcase, "buildid", buildid, validation.IntValidator()
        )
        return defer.succeed(None)
    ...
    def setBuildProperty(self, buildid, name, value, source):
        ...
        validation.verifyType(
            self.testcase, "source", source, validation.StringValidator()
        )
        return defer.succeed(None)
    ...
    def addStep(self, buildid, name):
        ...
        validation.verifyType(
            self.testcase, "name", name, validation.IdentifierValidator(50)
        )
        return defer.succeed((10, 1, name))

    def addStepURL(self, stepid, name, url):
        validation.verifyType(
            self.testcase, "stepid", stepid, validation.IntValidator()
        )
        validation.verifyType(self.testcase, "name", name, validation.StringValidator())
    ...
    def finishStep(self, stepid, results, hidden):
        ...
        validation.verifyType(
            self.testcase, "hidden", hidden, validation.BooleanValidator()
        )
        return defer.succeed(None)
    ...
    def addLog(self, stepid, name, type):
        ...
        validation.verifyType(self.testcase, "name", name, validation.StringValidator())
        validation.verifyType(
            self.testcase, "type", type, validation.IdentifierValidator(1)
        )
    ...
    def appendLog(self, logid, content):
        validation.verifyType(self.testcase, "logid", logid, validation.IntValidator())
        validation.verifyType(
            self.testcase, "content", content, validation.StringValidator()
        )
    ...
    def setWorkerState(self, workerid, paused, graceful):
        ...
        yield self.master.db.workers.set_worker_graceful(
            workerid=workerid, graceful=graceful
        )

    def set_worker_paused(self, workerid, paused, pause_reason=None):
        return self.master.db.workers.set_worker_paused(
            workerid, paused, pause_reason=pause_reason
        )
    ...
    def setBuildData(self, buildid, name, value, source):
        ...
        validation.verifyType(self.testcase, "name", name, validation.StringValidator())
        validation.verifyType(
            self.testcase, "value", value, validation.BinaryValidator()
        )
        validation.verifyType(
            self.testcase, "source", source, validation.StringValidator()
        )
    ...
    def addTestResultSet(
        ...
        validation.verifyType(
            self.testcase, "category", category, validation.StringValidator()
        )
        validation.verifyType(
...
class FakeDataConnector(service.AsyncMultiService):
    ...
    def resultspec_from_jsonapi(self, args, entityType, is_collection):
        return self.realConnector.resultspec_from_jsonapi(
            args, entityType, is_collection
        )
