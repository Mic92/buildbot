...
class TestDarcs(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_incremental_invalid_method(self):
        with self.assertRaises(config.ConfigErrors):
            darcs.Darcs(
                repourl="http://localhost/darcs", mode="incremental", method="fresh"
            )
    ...
    def test_mode_full_copy(self):
        self.setup_step(
            darcs.Darcs(repourl="http://localhost/darcs", mode="full", method="copy")
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["darcs", "--version"]).exit(0),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ...
            ExpectShell(
                workdir="source", command=["darcs", "pull", "--all", "--verbose"]
            ).exit(0),
            ExpectCpdir(
                fromdir="source", todir="build", log_environ=True, timeout=1200
            ).exit(0),
    ...
    def test_mode_incremental(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["darcs", "pull", "--all", "--verbose"]
            ).exit(0),
            ExpectShell(workdir="wkdir", command=["darcs", "changes", "--max-count=1"])
            .stdout("Tue Aug 20 09:18:41 IST 2013 abc@gmail.com")
            .exit(0),
    ...
    def test_mode_incremental_patched(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="build", command=["darcs", "pull", "--all", "--verbose"]
            ).exit(0),
            ExpectShell(workdir="build", command=["darcs", "changes", "--max-count=1"])
            .stdout("Tue Aug 20 09:18:41 IST 2013 abc@gmail.com")
            .exit(0),
    ...
    def test_mode_full_clobber_revision(self):
        self.setup_step(
            darcs.Darcs(
                repourl="http://localhost/darcs", mode="full", method="clobber"
            ),
            {"revision": "abcdef01"},
        )
        self.expect_commands(
