...
class TestShellCommandExecution(
    ...
    def test_run_nested_empty_command(self):
        self.setup_step(
            shell.ShellCommand(
                workdir="build", command=["trial", [], "-b", [], "buildbot.test"]
            )
        )
        self.expect_commands(
            ExpectShell(workdir="build", command=["trial", "-b", "buildbot.test"]).exit(
                0
            )
        )
        self.expect_outcome(result=SUCCESS, state_string="'trial -b ...'")
    ...
    def test_run_env(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="build", command="echo hello", env={"DEF": "HERE"}
            ).exit(0)
        )
    ...
    def test_run_env_override(self):
        self.setup_step(
            shell.ShellCommand(
                workdir="build", env={"ABC": "123"}, command="echo hello"
            ),
    ...
    def test_run_usePTY(self):
        self.setup_step(
            shell.ShellCommand(workdir="build", command="echo hello", usePTY=False)
        )
        self.expect_commands(
    ...
    def test_run_decodeRC(self, rc=1, results=WARNINGS, extra_text=" (warnings)"):
        self.setup_step(
            shell.ShellCommand(
                workdir="build", command="echo hello", decodeRC={1: WARNINGS}
            )
        )
        self.expect_commands(
...
class SetPropertyFromCommand(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_run_property_no_strip(self):
        self.setup_step(
            shell.SetPropertyFromCommand(property="res", command="cmd", strip=False)
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command="cmd").stdout("\n\nabcdef\n").exit(0)
        )
...
class PerlModuleTest(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_new_version_failed(self):
        ...
        self.expect_outcome(
            result=FAILURE, state_string="4 tests 3 passed 1 failed (failure)"
        )
        return self.run_step()
    ...
    def test_old_version_failed(self):
        ...
        self.expect_outcome(
            result=FAILURE, state_string="20 tests 17 passed 3 failed (failure)"
        )
        return self.run_step()
...
class WarningCountingShellCommand(
    ...
    def test_no_warnings(self):
        self.setup_step(
            shell.WarningCountingShellCommand(workdir="w", command=["make"])
        )
        self.expect_commands(
    ...
    def test_maxWarnCount(self):
        self.setup_step(
            shell.WarningCountingShellCommand(command=["make"], maxWarnCount=9)
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["make"])
            .stdout("warning: noo!\n" * 10)
    ...
    def test_warnExtractFromRegexpGroups(self):
        ...
        self.assertEqual(
            we(step, line, re.match(pat, line)), (exp_file, exp_lineNo, exp_text)
        )
