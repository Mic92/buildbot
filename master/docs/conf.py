...
try:
    ...
    sys.path.insert(
        2, os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir)
    )
    from buildbot.reporters.telegram import TelegramContact
    from buildbot.util.raml import RamlSpec
...
extlinks = {
    ...
    "contrib-src": (
        "https://github.com/buildbot/buildbot-contrib/tree/master/%s",
        "%s",
    ),
}
...
for raml_typename, raml_type in sorted(raml_spec.types.items()):
    ...
    if not os.path.exists(doc_path):
        raise RuntimeError(
            f"File {doc_path} for RAML type {raml_typename} does not exist"
        )
