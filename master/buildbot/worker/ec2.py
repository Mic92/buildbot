...
class EC2LatentWorker(AbstractLatentWorker):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not boto3:
            config.error(
                "The python module 'boto3' is needed to use a " "EC2LatentWorker"
            )

        if keypair_name is None:
            config.error(
                "EC2LatentWorker: 'keypair_name' parameter must be " "specified"
            )

        if security_name is None and not subnet_id:
            config.error(
                "EC2LatentWorker: 'security_name' parameter must be " "specified"
            )
        ...
        if spot_instance and price_multiplier is None and max_spot_price is None:
            raise ValueError(
                "You must provide either one, or both, of "
        ...
        self.block_device_map = (
            self.create_block_device_mapping(block_device_map)
            if block_device_map
            else None
        )
                    ...
                    try:
                        ...
                        if level == 0:
                            try:
                                ...
                                level = 1
                options.append(
                    [int_sort, alpha_sort, image.image_location, image.id, image]
                )
            if level:
    ...
    def _wait_for_instance(self):
        ...
        if self.instance.state["Name"] == RUNNING:
            ...
            self.failed_to_start(self.instance.id, self.instance.state["Name"])
            return (
                None  # This is just to silence warning, above line throws an exception
            )
