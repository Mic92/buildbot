...
class TestPropertyMap(unittest.TestCase):
    ...
    def testTempValueColonMinusSet(self):
        return self.doTestSimpleWithProperties(
            "%(prop_one:-missing)s", 2, prop_one=lambda b: 2
        )
    ...
    def testTempValuePlusSetSet(self):
        return self.doTestSimpleWithProperties(
            "%(prop_one:+set)s", "set", prop_one=lambda b: 2
        )
...
class TestPropertiesMixin(unittest.TestCase):
    ...
    def test_setProperty(self):
        ...
        self.mp.properties.setProperty.assert_called_with(
            "abc", "def", "src", runtime=True
        )
...
class TestProperty(unittest.TestCase):
    ...
    def testPropMult(self):
        ...
        result = yield self.build.render(
            Property("do-tests") * Interpolate("%(prop:do-tests)s")
        )
...
class Renderer(unittest.TestCase):
    ...
    def test_renderer_recursive_with_state(self):
        ...
        def rend(p, arg, kwarg="y"):
            return Interpolate(
                "x-%(prop:x)s-%(kw:arg)s-%(kw:kwarg)s", arg=arg, kwarg=kwarg
            )
    ...
    def test_interpolate_worker(self):
        self.build.workerforbuilder.worker.info.setProperty(
            "test", "testvalue", "Worker"
        )
...
class Compare(unittest.TestCase):
    def test_WithProperties_lambda(self):
        ...
        self.assertEqual(
            WithProperties("%(key)s", key=rend), WithProperties("%(key)s", key=rend)
        )
        self.assertNotEqual(
    ...
    def test_WithProperties_positional(self):
        self.assertNotEqual(
            WithProperties("%s", "key"), WithProperties("%s", "otherkey")
        )
    ...
    def test_Interpolate_constant(self):
        self.assertNotEqual(
            Interpolate("some text here"), Interpolate("and other text there")
        )
