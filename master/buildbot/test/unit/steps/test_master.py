...
class TestMasterShellCommand(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_env_subst(self):
        ...
        self.expect_commands(
            ExpectMasterShell(exp_argv).env({"HELLO": "hello"}).exit(0)
        )
    ...
    def test_env_list_subst(self):
        ...
        self.setup_step(
            master.MasterShellCommand(
                command="true", env={"HELLO": ["${WORLD}", "${LIST}"]}
            )
        )
        ...
        self.expect_commands(
            ExpectMasterShell(exp_argv).env({"HELLO": exp_env}).exit(0)
        )
    ...
    def test_constr_args_descriptionSuffix(self):
        ...
        self.expect_commands(
            ExpectMasterShell(exp_argv).workdir("build").env({"a": "b"}).exit(0)
        )
