...
class PerforceExtractor(SourceStampExtractor):
    ...
    def readPatch(self, diff, patchlevel):
        ...
        for line in diff.split("\n"):
            m = re.search(
                "==== //depot/" + self.branch + r"/([\w/\.\d\-_]+)#(\d+) -", line
            )
            if m:
...
class RemoteTryPP(protocol.ProcessProtocol):
    ...
    def processEnded(self, reason):
        ...
        if sig is not None or rc != 0:
            self.d.errback(
                RuntimeError(f"remote 'buildbot tryserver' failed: sig={sig}, rc={rc}")
            )
            return
...
class Try(pb.Referenceable):
    ...
    def deliver_job_pb(self):
        ...
        d = f.login(
            credentials.UsernamePassword(unicode2bytes(user), unicode2bytes(passwd))
        )
    ...
    def deliverJob(self):
        ...
        raise RuntimeError(
            f"unknown connecttype '{self.connect}', should be 'ssh' or 'pb'"
        )
    ...
    def getStatus(self):
        ...
        if not wait:
            ...
            if not self.buildsetStatus:
                output(
                    "try scheduler on the master does not have the builder configured"
                )
                return None
