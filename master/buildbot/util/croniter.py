...
special_day_of_week_or_month_re = re.compile(
    (r"^(?P<pre>((?P<he>(({WEEKDAYS})(-({WEEKDAYS}))?)").format(
        WEEKDAYS=WEEKDAYS_RE_PART
    )
    + (r"|(({MONTHS})(-({MONTHS}))?)|\w+)#)|l)(?P<last>\d+)$").format(
        MONTHS=MONTHS_RE_PART
    )
)
    ...
    def __init__(self, expr_format, start_time=time()):
        ...
        for index, expr in enumerate(self.exprs):
            ...
            while field_list:
                ...
                nth = None
                special_day_of_week_or_month_re_match = (
                    special_day_of_week_or_month_re.match(field)
                )
                ...
                # and 3 is last.
                if (
                    index == self.DAY_OF_WEEK_FIELD_INDEX
                    and special_day_of_week_or_month_re_match
                ):
                ...
                t = re.sub(
                    r"^\*(\/.+)$",
                    rf"{self.RANGES[index][0]}-{self.RANGES[index][1]}\1",
                    field,
                )
                ...
                if not m:
                    ...
                    t = re.sub(
                        r"^(.+)\/(.+)$", rf"\1-{self.RANGES[index][1]}/\2", field
                    )
                    m = step_search_re.search(t)
    ...
    def _calc(self, now, expanded, nth_weekday_of_month, is_prev):
        ...
        if is_prev:
            procs = [
                ...
                proc_hour,
                (
                    proc_day_of_week_nth
                    if self.nth_weekday_of_month
