...
class BuilderMixin:
    ...
    def makeBuilder(
        self, name="bldr", patch_random=False, noReconfig=False, **config_kwargs
    ):
        """Set up C{self.bldr}"""
        ...
        if patch_random:
            ...
            self.patch(
                random, "choice", lambda lst: sorted(lst, key=lambda m: m.name)[-1]
            )
...
class TestBuilder(TestReactorMixin, BuilderMixin, unittest.TestCase):
    ...
    def makeBuilder(
        self, patch_random=False, startBuildsForSucceeds=True, **config_kwargs
    ):
...
class TestGetOldestRequestTime(TestReactorMixin, BuilderMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_rows = [
            ...
            fakedb.BuildRequest(
                id=555, submitted_at=2800, builderid=182, buildsetid=11
            ),
        ]
...
class TestGetHighestPriority(TestReactorMixin, BuilderMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_rows = [
            ...
            fakedb.Builder(id=78, name="bldr2"),
            fakedb.BuildRequest(
                id=111, submitted_at=1000, builderid=77, buildsetid=11, priority=0
            ),
            fakedb.BuildRequest(
            ...
            fakedb.BuildRequestClaim(brid=222, masterid=master_id, claimed_at=2001),
            fakedb.BuildRequest(
                id=333, submitted_at=3000, builderid=77, buildsetid=11, priority=5
            ),
            fakedb.BuildRequest(
                id=444, submitted_at=3001, builderid=77, buildsetid=11, priority=3
            ),
...
class TestReconfig(TestReactorMixin, BuilderMixin, unittest.TestCase):
    ...
    def test_reconfig(self):
        yield self.makeBuilder(
            description="Old", project="old_project", tags=["OldTag"]
        )
    ...
    def test_reconfig_changed(
        self, name, new_desc, new_project, new_tags, expect_project_id
    ):
        yield self.makeBuilder(
            description="Old", project="old_project", tags=["OldTag"]
        )
        ...
        self.master.data.updates.updateBuilderInfo = (
            ...
            desc_html,
            projectid,
            tags: builder_updates.append(
                (builderid, desc, desc_format, desc_html, projectid, tags)
            )
        )

        yield self.bldr.reconfigServiceWithBuildbotConfig(mastercfg)
        self.assertEqual(
            builder_updates, [(1, new_desc, None, None, expect_project_id, new_tags)]
        )
