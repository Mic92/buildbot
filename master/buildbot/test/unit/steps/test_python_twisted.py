...
class Trial(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_run_singular(self):
        self.setup_step(
            python_twisted.Trial(workdir="build", tests="testname", testpath=None)
        )
        self.expect_commands(
            ExpectShell(
                workdir="build",
    ...
    def test_renderable_properties(self):
        self.setup_step(
            python_twisted.Trial(
                workdir="build", tests=Property("test_list"), testpath=None
            )
        )
    ...
    def test_randomly(self):
        self.setup_step(
            python_twisted.Trial(
                workdir="build", randomly=True, tests="testname", testpath=None
            )
        )
    ...
    def test_run_jobs(self):
        ...
        self.setup_step(
            python_twisted.Trial(
                workdir="build", tests="testname", testpath=None, jobs=2
            )
        )
...
class HLint(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_run_ok(self):
        self.setup_step(
            python_twisted.HLint(workdir="build"), build_files=["foo.xhtml"]
        )
        self.expect_commands(
            ExpectShell(
                workdir="build",
