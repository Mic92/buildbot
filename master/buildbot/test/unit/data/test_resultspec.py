...
class ResultSpec(unittest.TestCase):
    ...
    def test_apply_details_fields(self):
        ...
        self.assertEqual(
            resultspec.ResultSpec(fields=["name"]).apply(data), {"name": "clyde"}
        )
        self.assertEqual(
    ...
    def test_apply_collection_fields(self):
        ...
        self.assertEqual(
            resultspec.ResultSpec(fields=["a"]).apply(data), mklist("a", 1, 2)
        )
        self.assertEqual(
    ...
    def test_apply_ordering(self):
        ...
        self.assertEqual(
            resultspec.ResultSpec(order=["-name"]).apply(data), list(reversed(exp))
        )
    ...
    def test_apply_ordering_multi(self):
        ...
        self.assertListResultEqual(
            resultspec.ResultSpec(order=["ln", "fn"]).apply(data), exp
        )
        exp = base.ListResult(
            mklist(
                ("fn", "ln"),
    ...
    def test_sort_null_datetimefields(self):
        data = mklist(
            ("fn", "ln"), ("albert", datetime.datetime(1, 1, 1)), ("cedric", None)
        )

        exp = mklist(
            ("fn", "ln"), ("cedric", None), ("albert", datetime.datetime(1, 1, 1))
        )
    ...
    def do_test_pagination(self, bareList):
        ...
        self.assertListResultEqual(
            ...
            base.ListResult(
                mklist("x", *list(range(111, 121))), offset=10, total=30, limit=10
            ),
        )
        self.assertListResultEqual(
            resultspec.ResultSpec(offset=20, limit=15).apply(data),
            base.ListResult(
                mklist("x", *list(range(121, 131))), offset=20, total=30, limit=15
            ),
        )  # off the end
    ...
    def test_pagination_prepaginated(self):
        ...
        self.assertListResultEqual(
            ...
            base.ListResult(
                mklist("x", *list(range(10, 20))), offset=10, total=30, limit=10
            ),
        )
...
class Comparator(unittest.TestCase):
    def test_noneComparator(self):
        self.assertNotEqual(
            NoneComparator(None), NoneComparator(datetime.datetime(1, 1, 1))
        )
        self.assertNotEqual(
            NoneComparator(datetime.datetime(1, 1, 1)), NoneComparator(None)
        )
        self.assertLess(
            NoneComparator(None), NoneComparator(datetime.datetime(1, 1, 1))
        )
