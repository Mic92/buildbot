...
class GitMixin:
    def setupGit(self, logname=None):
        ...
        if self.sshHostKey is not None and self.sshPrivateKey is None:
            config.error(
                f"{logname}: sshPrivateKey must be provided in order use sshHostKey"
            )
        ...
        if self.sshHostKey is not None and self.sshKnownHosts is not None:
            config.error(
                f"{logname}: only one of sshKnownHosts and sshHostKey can be provided"
            )
    ...
    def adjustCommandParamsForSshPrivateKey(
        ...
        if self.supportsSshPrivateKeyAsConfigOption:
            ...
            if sshWrapperPath is None:
                raise RuntimeError(
                    "Only SSH wrapper script is supported but path not given"
                )
...
def ensureSshKeyNewline(privateKey: str) -> str:
    ...
    if (
        privateKey.endswith("\n")
        or privateKey.endswith("\r")
        or privateKey.endswith("\r\n")
    ):
        return privateKey
    return privateKey + "\n"
