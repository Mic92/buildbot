...
class GitHubEventHandler(PullRequestMixin):
    ...
    def handle_push(self, payload, event):
        ...
        changes = self._process_change(
            payload, user, repo, repo_url, project, event, properties
        )
    ...
    def _process_change(
        self, payload, user, repo, repo_url, project, event, properties
    ):
        """
...
class GitHubHandler(BaseHookHandler):
    def __init__(self, master, options):
        ...
        handler = klass(
            options.get("secret", None), options.get("strict", False), **klass_kwargs
        )
