...
class RepoURL(unittest.TestCase):
    ...
    def test_parse3(self):
        self.oneTest(
            {
                "a": "repo download test/bla 564/12 repo download "
...
class TestRepo(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def expectnoClobber(self):
        ...
        self.expect_commands(
            ExpectStat(file="wkdir/.repo", log_environ=self.logEnviron).exit(0)
        )
    ...
    def expectRepoSync(
        ...
        commands = (
            ...
            + [
                self.ExpectShell(
                    command=["repo", "sync", "--force-sync"] + syncoptions
                ),
                self.ExpectShell(
    ...
    def test_manifest_override(self):
        ...
        """
        self.mySetupStep(
            manifestOverrideUrl="http://u.rl/test.manifest", syncAllBranches=True
        )
        self.expectClobber()
        override_commands = [
        ...
        self.expectRepoSync(
            which_fail=2, syncoptions=[], override_commands=override_commands
        )
        return self.myRunStep()
    ...
    def test_tarball(self):
        ...
        self.expectClobber()
        self.expect_commands(
            self.ExpectShell(command=["tar", "-xvf", "/tarball.tar"]).exit(0)
        )
        self.expectRepoSync()
        self.expect_commands(
    ...
    def test_create_tarball(self):
        ...
        self.expect_commands(
            ...
            .exit(1),
            self.ExpectShell(
                command=["tar", "-z", "-cvf", "/tarball.tgz", ".repo"]
            ).exit(0),
        )
        return self.myRunStep()
    ...
    def test_update_tarball_fail2(self, suffix="tar", option=None):
        ...
        self.expect_commands(
            ...
            self.ExpectShell(command=["rm", "-f", "/tarball.tar"]).exit(0),
            self.ExpectShell(command=["repo", "download", "test/bla", "564/12"]).exit(
                0
            ),
        )
        return self.myRunStep()
    ...
    def test_repo_downloads2(self):
        ...
        self.build.setProperty(
            "repo_download2", "repo download test/bla2 565/12", "test"
        )
        self.expectnoClobber()
        self.expectRepoSync()
        self.expect_commands(
            ...
            self.ExpectShell(command=["repo", "download", "test/bla2", "565/12"]).exit(
                0
            ),
        )
        return self.myRunStep()
    ...
    def test_repo_download_manifest(self):
        ...
        self.build.setProperty("repo_download", "repo download test/bla 564/12", "test")
        self.build.setProperty(
            "repo_download2", "repo download manifest 565/12", "test"
        )
        self.expectnoClobber()
        self.expect_commands(
            self.ExpectShell(
    ...
    def test_update_doublefail2(self):
        ...
        self.shouldRetry = True
        return self.myRunStep(
            result=FAILURE, state_string="repo failed at: repo sync (failure)"
        )
    ...
    def test_update_doublefail3(self):
        ...
        self.shouldRetry = True
        return self.myRunStep(
            result=FAILURE, state_string="repo failed at: repo init (failure)"
        )
