    ...
    def __init__(self, fmtstring, *args, **lambda_subs):
        ...
        if not self.args:
            ...
            for key, val in self.lambda_subs.items():
                if not callable(val):
                    raise ValueError(
                        f'Value for lambda substitution "{key}" must be callable.'
                    )
        elif lambda_subs:
            raise ValueError(
...
class _SecretRenderer:
    ...
    def getRenderingFor(self, properties):
        ...
        if secret_detail is None:
            raise KeyError(
                f"secret key {self.secret_name} is not found in any provider"
            )
...
class Interpolate(RenderableOperatorsMixin, util.ComparableMixin):
    ...
    def __init__(self, fmtstring, *args, **kwargs):
        ...
        if self.args and self.kwargs:
            config.error(
                "Interpolate takes either positional or keyword "
    ...
    def _parse_substitution_prop(self, arg):
        ...
        if not Interpolate.identifier_re.match(prop):
            config.error(
                f"Property name must be alphanumeric for prop Interpolation '{arg}'"
            )
            prop = repl = None
    ...
    def _parse_substitution_src(self, arg):
        ...
        try:
            ...
            try:
                ...
                config.error(
                    "Must specify both codebase and attribute for "
        ...
        if not Interpolate.identifier_re.match(attr):
            config.error(
                f"Attribute must be alphanumeric for src Interpolation '{arg}'"
            )
            codebase = attr = repl = None
