...
class TestCVS(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_mode_full_clean_and_login(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir="wkdir", command=["cvs", "-z3", "update", "-dP"]).exit(
                0
            ),
        )
    ...
    def test_mode_full_clean_timeout(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", timeout=1, command=["cvs", "--version"]).exit(
                0
            ),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ExpectUploadFile(
            ...
            ExpectShell(
                workdir="wkdir", timeout=1, command=["cvs", "-z3", "update", "-dP"]
            ).exit(0),
        )
    ...
    def test_mode_full_copy(self):
        ...
        self.expect_commands(
            ...
            .upload_string("/file/1.1/Fri May 17 23:20:00//\nD\n")
            .exit(0),
            ExpectShell(workdir="source", command=["cvs", "-z3", "update", "-dP"]).exit(
                0
            ),
            ExpectCpdir(
    ...
    def test_command_fails(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["cvs", "--version"]).exit(128)
        )
