...
class TestVaultHvac(RunMasterBase):
    ...
    def setup_config(self, secret_specifier):
        ...
        f = BuildFactory()
        f.addStep(
            ShellCommand(command=Interpolate(f"echo {secret_specifier} | base64"))
        )
    ...
    def start_container(self, image_tag):
        try:
            ...
            raise SkipTest(
                "Vault integration needs docker environment to be setup"
            ) from e
    ...
    def test_key(self, image_tag):
        yield self.do_secret_test(
            image_tag, "%(secret:key|value)s", "<key|value>", "word"
        )
    ...
    def test_nested_key(self, image_tag):
        yield self.do_secret_test(
            image_tag, "%(secret:key1/key2|id)s", "<key1/key2|id>", "val"
        )
