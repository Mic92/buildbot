...
class TestTrigger(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_sourceStamp_and_updateSourceStamp(self):
        with self.assertRaises(config.ConfigErrors):
            trigger.Trigger(
                schedulerNames=["c"], sourceStamp={"x": 1}, updateSourceStamp=True
            )
    ...
    def test_updateSourceStamp_and_alwaysUseLatest(self):
        with self.assertRaises(config.ConfigErrors):
            trigger.Trigger(
                schedulerNames=["c"], updateSourceStamp=True, alwaysUseLatest=True
            )

    def test_sourceStamp_and_alwaysUseLatest(self):
        with self.assertRaises(config.ConfigErrors):
            trigger.Trigger(
                schedulerNames=["c"], sourceStamp={"x": 1}, alwaysUseLatest=True
            )
    ...
    def test_updateSourceStamp(self):
        yield self.setup_step(
            ...
            sourcestampsInBuild=[
                FakeSourceStamp(codebase="", repository="x", revision=11111)
            ],
    ...
    def test_updateSourceStamp_prop_false(self):
        yield self.setup_step(
            trigger.Trigger(
                schedulerNames=["a"], updateSourceStamp=properties.Property("usess")
            ),
            sourcestampsInBuild=[
                FakeSourceStamp(codebase="", repository="x", revision=11111)
            ],
    ...
    def test_waitForFinish(self):
        yield self.setup_step(
            trigger.Trigger(schedulerNames=["a", "b"], waitForFinish=True)
        )
    ...
    def test_set_properties(self):
        ...
        self.expect_outcome(result=SUCCESS)
        self.expectTriggeredWith(
            a=(False, [], {"x": (1, "Trigger"), "y": (2, "Trigger")})
        )
        yield self.run_step()
    ...
    def test_set_properties_prop(self):
        ...
        self.expect_outcome(result=SUCCESS)
        self.expectTriggeredWith(
            a=(False, [], {"x": ("xxx", "Trigger"), "y": (2, "Trigger")})
        )
        yield self.run_step()
    ...
    def test_copy_properties(self):
        yield self.setup_step(
            trigger.Trigger(schedulerNames=["a"], copy_properties=["a", "b"])
        )
        ...
        self.expect_outcome(result=SUCCESS)
        self.expectTriggeredWith(
            a=(False, [], {"a": ("A", "Trigger"), "b": ("B", "Trigger")})
        )
        yield self.run_step()
