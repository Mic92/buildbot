...
class ProtocolCommandMsgpack(ProtocolCommandBase):
    ...
    def protocol_send_update_message(self, message):
        d = self.protocol.get_message_result(
            {"op": "update", "args": message, "command_id": self.command_id}
        )
...
class BuildbotWebSocketClientProtocol(WebSocketClientProtocol):
    ...
    def onConnecting(self, transport_details):
        ...
        auth_header = encode_http_authorization_header(
            self.factory.name, self.factory.password
        )
        ...
        try:
            ...
            for setting in [
                ...
                if setting not in msg["args"]:
                    raise KeyError(
                        "message did not contain obligatory settings for worker"
                    )
    ...
    def call_interrupt_command(self, msg):
        ...
        try:
            ...
            # send an instance, on which get_message_result will be called
            yield self.factory.buildbot_bot.interrupt_command(
    ...
    def onMessage(self, payload, isBinary):
        ...
        if msg["op"] == "print":
            ...
            if "is_exception" in msg:
                self.seq_num_to_waiters_map[seq_number].errback(
                    RemoteWorkerError(msg["result"])
                )
            else:
