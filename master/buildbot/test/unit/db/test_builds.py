...
class Tests(interfaces.InterfaceTests):
    ...
    def test_getBuilds_builderid(self):
        ...
        for bdict in bdicts:
            validation.verifyDbDict(self, "dbbuilddict", bdict)
        self.assertEqual(
            sorted(bdicts, key=lambda bd: bd["id"]), [self.threeBdicts[51]]
        )
    ...
    def test_getBuilds_complete(self):
        ...
        for bdict in bdicts:
            validation.verifyDbDict(self, "dbbuilddict", bdict)
        self.assertEqual(
            sorted(bdicts, key=lambda bd: bd["id"]), [self.threeBdicts[52]]
        )
    ...
    def test_addBuild_existing(self):
        ...
        yield self.insert_test_data(
            ...
            + [
                fakedb.Build(
                    number=10, buildrequestid=41, builderid=77, masterid=88, workerid=13
                ),
            ]
        )
        id, number = yield self.db.builds.addBuild(
...
class RealTests(Tests):
    ...
    def test_getBuilds_resultSpecFilterNeTwoValues(self):
        ...
        for bdict in bdicts:
            validation.verifyDbDict(self, "dbbuilddict", bdict)
        self.assertEqual(
            sorted(bdicts, key=lambda bd: bd["id"]), [self.threeBdicts[50]]
        )
    ...
    def test_getBuilds_resultSpecFilterContainsTwoValues(self):
        rs = resultspec.ResultSpec(
            filters=[
                resultspec.Filter("state_string", "contains", ["build 5", "build 6"])
            ]
        )
        rs.fieldMapping = {"state_string": "builds.state_string"}
        yield self.insert_test_data(self.backgroundData + self.threeBuilds)
...
class TestRealDB(
    unittest.TestCase, connector_component.ConnectorComponentMixin, RealTests
):
    @defer.inlineCallbacks
    def setUp(self):
        yield self.setUpConnectorComponent(
