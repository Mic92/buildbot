    ...
    def addService(self, service):
        if service.name is not None:
            if service.name in self.namedServices:
                raise RuntimeError(
                    "cannot have two services with same name" f" '{service.name}'"
                )
    ...
    def stopService(self):
        ...
        if self.active:
            ...
            try:
                ...
                msg = (
                    f"Caught exception while deactivating ClusteredService({self.name})"
                )
                log.err(e, _why=msg)
        ...
        if removed_names or added_names:
            ...
            for n in added_names:
                ...
                if hasattr(child, "objectid"):
                    class_name = (
                        f"{child.__class__.__module__}.{child.__class__.__name__}"
                    )
