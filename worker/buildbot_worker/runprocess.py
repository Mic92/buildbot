...
class RunProcessPP(protocol.ProcessProtocol):
    ...
    def connectionMade(self):
        ...
        if self.command.useProcGroup:
            if self.debug:
                self.command.log_msg(
                    "pid {0} set as subprocess pgid".format(self.transport.pid)
                )
...
class RunProcess(object):
    ...
    def __init__(
        ...
        for name, filevalue in self.logfiles.items():
            ...
            w = LogFileWatcher(
                self, name, os.path.join(self.workdir, filename), follow=follow
            )
    ...
    def start(self):
        ...
        try:
            ...
            self.send_update(
                [("stderr", "error in RunProcess._startCommand (%s)\n" % str(e))]
            )
    ...
    def _startCommand(self):
        ...
        if isinstance(self.command, bytes):
            ...
            if runtime.platformType == "win32" and not (
                bytes2unicode(self.command[0], self.unicode_encoding)
                .lower()
                .endswith(".exe")
    ...
    def _spawnAsBatch(self, processProtocol, executable, args, env, path, usePTY):
        ...
        return reactor.spawnProcess(
            processProtocol, executable, argv, env, path, usePTY=usePTY
        )
    ...
    def finished(self, sig, rc):
        ...
        self.log_msg(
            (
                "command finished with signal {0}, exit code {1}, "
                + "elapsedTime: {2:0.6f}"
            ).format(sig, rc, self.elapsedTime)
        )
        for w in self.logFileWatchers:
    ...
    def cleanUp(self, hit):
        ...
        if self.deferred:
            ...
            self.killTimer = self._reactor.callLater(
    ...
    def sendSig(self, interruptSignal):
        ...
        if not hit and self.useProcGroup and runtime.platformType == "posix":
            ...
            if sig is None:
                ...
                self.log_msg(
                    "trying to kill process group {0}".format(self.process.pgid)
                )
                try:
                    ...
                    self.log_msg(
                        "failed to kill process group (ignored): {0}".format(
                            (sys.exc_info()[1])
                        )
                    )
        ...
        if not hit:
            try:
                self.log_msg(
                    "trying process.signalProcess('{0}')".format(interruptSignal)
                )
