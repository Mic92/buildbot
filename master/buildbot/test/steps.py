...
class ExpectStat(Expect):
    ...
    def stat(
        ...
        self.update(
            "stat", [mode, inode, dev, nlink, uid, gid, size, atime, mtime, ctime]
        )
        return self
...
class ExpectCpdir(Expect):
    def __init__(
        self, fromdir=None, todir=None, log_environ=None, timeout=None, max_time=None
    ):
...
class TestBuildStepMixin:
    ...
    def run_step(self):
        ...
        if result != self.exp_result:
            msg = (
                f"unexpected result from step; expected {self.exp_result}, got {result}"
            )
