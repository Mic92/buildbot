...
class FailingBuildsetCanceller(BuildbotService):
    ...
    def reconfigService(self, name, filters):
        self.filters = FailingBuildsetCanceller.filter_tuples_to_filter_set_object(
            filters
        )
    ...
    def check_filters(cls, filters):
        if not isinstance(filters, list):
            config.error(
                f"{cls.__name__}: The filters argument must be a list of tuples"
            )
        ...
        for filter in filters:
            ...
            try:
                ...
                config.error(
                    f"{cls.__name__}: When processing filter builders: {str(e)}"
                )
    ...
    def _on_build_finished(self, key, build):
        ...
        for br in all_bs_buildrequests:
            ...
            if (
                builders_to_cancel is not None
                and br_builder["name"] not in builders_to_cancel
            ):
                continue
            ...
            self.master.data.control(
                "cancel", {"reason": reason}, ("buildrequests", str(brid))
            )
