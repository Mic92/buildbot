...
STATUS_CORE_API_URL = (
    "/rest/api/1.0/projects/{proj_key}/repos/{repo_slug}/commits/{sha}/builds"
)
        ...
        for sourcestamp in sourcestamps:
            try:
                ...
                if res.code not in (HTTP_PROCESSED,):
                    ...
                    log.msg(
                        f"{res.code}: Unable to send Bitbucket Server status: "
                        f"{content}"
                    )
                elif self.verbose:
...
class BitbucketServerCoreAPIStatusPush(ReporterBase):
    ...
    def createStatus(
        ...
        _url = STATUS_CORE_API_URL.format(
            proj_key=proj_key, repo_slug=repo_slug, sha=sha
        )
    ...
    def sendMessage(self, reports):
        ...
        if self.status_name:
            ...
            if parent_name:
                status_name = f"{parent_name} #{build['parentbuild']['number']} \u00BB {status_name}"
