...
class TestRunProcess(TestReactorMixin, LoggingMixin, unittest.TestCase):
    ...
    def run_process(
        self, command, override_kill_success=True, override_is_dead=True, **kwargs
    ):
        self.run_process_obj = RunProcess(self.reactor, command, "/workdir", **kwargs)
        self.run_process_obj.get_os_env = lambda: {"OS_ENV": "value"}
        self.run_process_obj.send_signal = mock.Mock(
            side_effect=lambda sig: override_kill_success
        )
    ...
    def test_no_output(self):
        ...
        self.assertEqual(
            ...
            (
                "cmd",
                ["cmd"],
                {"OS_ENV": "value", "PWD": os.path.abspath("/workdir")},
                "/workdir",
            ),
        )
    ...
    def test_io_timeout(self, name, wait1, wait2, timed_out, had_stdout, had_stderr):
        d = self.run_process(
            ["cmd"], collect_stdout=True, collect_stderr=True, io_timeout=5
        )
    ...
    def test_runtime_timeout(self, name, wait, timed_out):
        d = self.run_process(
            ["cmd"], collect_stdout=True, collect_stderr=True, runtime_timeout=5
        )
