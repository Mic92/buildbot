...
class TestMessageFormatting(unittest.TestCase):
    ...
    def test_get_detected_status_text_other(self):
        ...
        self.assertEqual(
            message.get_detected_status_text(["problem"], RETRY, None), "retry build"
        )
        self.assertEqual(
...
class MessageFormatterTestBase(TestReactorMixin, unittest.TestCase):
    def setUp(self):
        ...
        self.master = fakemaster.make_master(
            self, wantData=True, wantDb=True, wantMq=True
        )

    def setup_db(
        self, results1, results2, with_steps=False, extra_build_properties=None
    ):
            ...
            self.db.insert_test_data(
                [
                    fakedb.BuildProperty(
                        buildid=build_id, name="workername", value="wrkr"
                    ),
                    fakedb.BuildProperty(
                        buildid=build_id, name="reason", value="because"
                    ),
                ]
            )
        ...
        if with_steps:
            self.db.insert_test_data(
                [
                    fakedb.Step(
                        id=151, buildid=21, number=1, results=SUCCESS, name="first step"
                    ),
                    fakedb.Step(
                        id=152,
                        buildid=21,
                    ...
                    fakedb.Step(
                        id=153, buildid=21, number=3, results=SUCCESS, name="third step"
                    ),
                    fakedb.Log(
                        id=251,
                        stepid=152,
...
class TestMessageFormatter(MessageFormatterTestBase):
    def test_want_properties_deprecated(self):
        with assertProducesWarning(
            DeprecatedApiWarning, "wantProperties has been deprecated"
        ):
    ...
    def test_want_steps_deprecated(self):
        with assertProducesWarning(
            DeprecatedApiWarning, "wantSteps has been deprecated"
        ):
    ...
    def test_want_logs_deprecated(self):
        with assertProducesWarning(
            DeprecatedApiWarning, "wantLogs has been deprecated"
        ):
