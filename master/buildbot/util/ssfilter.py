...
def extract_filter_values(values, filter_name):
    if not isinstance(values, (list, str)):
        raise ValueError(
            f"Values of filter {filter_name} must be list of strings or a string"
        )
...
def extract_filter_values_branch(values, filter_name):
    if not isinstance(values, (list, str, type(None))):
        raise ValueError(
            f"Values of filter {filter_name} must be list of strings, "
            "a string or None"
        )
    if isinstance(values, (str, type(None))):
        ...
        for value in values:
            if not isinstance(value, (str, type(None))):
                raise ValueError(
                    f"Value of filter {filter_name} must be string or None"
                )
    return values
    ...
    if isinstance(values, (str, re.Pattern)):
        ...
        for value in values:
            if not isinstance(value, (str, re.Pattern)):
                raise ValueError(
                    f"Value of filter {filter_name} must be string or regex"
                )
    return values
...
def _create_property_filters(eq, not_eq, regex, not_regex, arg_prefix):
    ...
    if eq is not None:
        ...
        filters += [
            _FilterExactMatch(prop, values) for prop, values in values_dict.items()
        ]
    ...
    if not_regex is not None:
        values_dict = extract_filter_values_dict_regex(
            not_regex, arg_prefix + "_not_re"
        )
        filters += [
            _FilterRegexInverse(prop, values) for prop, values in values_dict.items()
        ]
