...
class TestSecretInVaultAuthenticatorToken(
    unittest.TestCase, TestSecretInVaultAuthenticator
):
    def setUp(self):
        if hvac is None:
...
class TestSecretInVaultAuthenticatorApprole(
    unittest.TestCase, TestSecretInVaultAuthenticator
):
    def test_authenticate(self):
...
class TestSecretInHashiCorpVaultKvSecretProvider(ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_check_config_authenticator(self):
        with self.assertRaisesConfigError(
            "authenticator must be instance of VaultAuthenticator"
        ):
