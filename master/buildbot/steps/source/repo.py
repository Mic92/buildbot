...
class Repo(Source):
    ...
    def _repoCmd(self, command, abandonOnFailure=True, **kwargs):
        return self._Cmd(
            ["repo"] + command, abandonOnFailure=abandonOnFailure, **kwargs
        )
    ...
    def doRepoSync(self, forceClobber=False):
        ...
        if self.manifestOverrideUrl:
            ...
            local_path = self.build.path_module.join(
                self.workdir, self.manifestOverrideUrl
            )
            local_file = yield self.pathExists(local_path)
            if local_file:
                yield self._Cmd(
                    ["cp", "-f", self.manifestOverrideUrl, "manifest_override.xml"]
                )
            else:
                yield self._Cmd(
                    ["wget", self.manifestOverrideUrl, "-O", "manifest_override.xml"]
                )
            yield self._Cmd(
    ...
    def doRepoDownloads(self):
        ...
        for download in self.repoDownloads:
            ...
            while retry > 0:
                ...
                retry -= 1
                yield self.stdio_log.addStderr(
                    f"failed downloading changeset {download}\n"
                )
            ...
            if self.lastCommand.didFail() or self._findErrorMessages(
                self.cherry_pick_error_re
            ):
