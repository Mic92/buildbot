...
class TestGit(
    ...
    def test_mode_full_clean(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "clean", "-f", "-f", "-d"]
            ).exit(0),
            ExpectShell(
                workdir="wkdir",
                command=[
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "checkout", "-f", "FETCH_HEAD"]
            ).exit(0),
            ExpectShell(workdir="wkdir", command=["git", "rev-parse", "HEAD"])
            .stdout("f6ad368298bd941e934a41f3babc827b2aa95a1d")
            .exit(0),
    ...
    def test_mode_full_clean_ssh_host_key_2_10_abs_workdir(self):
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectStat(
                file="/myworkdir/workdir/.buildbot-patched", log_environ=True
            ).exit(1),
            ExpectMkdir(dir=ssh_workdir, log_environ=True).exit(0),
            ExpectDownloadFile(
                blocksize=32768,
            ...
            ExpectShell(
                workdir=workdir, command=["git", "clean", "-f", "-f", "-d"]
            ).exit(0),
            ExpectShell(
                workdir=workdir,
                command=[
            ...
            ExpectShell(
                workdir=workdir, command=["git", "checkout", "-f", "FETCH_HEAD"]
            ).exit(0),
    ...
    def test_mode_full_clean_timeout(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", timeout=1, command=["git", "rev-parse", "HEAD"]
            )
            .stdout("f6ad368298bd941e934a41f3babc827b2aa95a1d")
            .exit(0),
        )
    ...
    def test_mode_full_clean_patch(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "update-index", "--refresh"]
            ).exit(0),
            ExpectShell(
                workdir="wkdir",
    ...
    def test_mode_full_clean_branch(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "checkout", "-B", "test-branch"]
            ).exit(0),
            ExpectShell(workdir="wkdir", command=["git", "rev-parse", "HEAD"])
            .stdout("f6ad368298bd941e934a41f3babc827b2aa95a1d")
            .exit(0),
    ...
    def test_mode_incremental_oldworker(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", interrupt_signal="TERM", command=["git", "--version"]
            )
            .stdout("git version 1.7.5")
            .exit(0),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
    ...
    def test_version_format(self):
        self.setup_step(
            self.stepClass(
                repourl="http://github.com/buildbot/buildbot.git", mode="incremental"
            )
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["git", "--version"])
    ...
    def test_mode_full_fresh(self):
        ...
        self.expect_commands(
            ...
            ExpectListdir(dir="wkdir").files([".git"]).exit(0),
            ExpectShell(
                workdir="wkdir", command=["git", "clean", "-f", "-f", "-d", "-x"]
            ).exit(0),
            ExpectShell(
                workdir="wkdir",
                command=[
    ...
    def test_mode_incremental_given_revision(self):
        ...
        self.expect_commands(
            ...
            ExpectListdir(dir="wkdir").files([".git"]).exit(0),
            ExpectShell(
                workdir="wkdir", command=["git", "cat-file", "-e", "abcdef01"]
            ).exit(0),
            ExpectShell(
                workdir="wkdir", command=["git", "checkout", "-f", "abcdef01"]
            ).exit(0),
            ExpectShell(workdir="wkdir", command=["git", "rev-parse", "HEAD"])
            .stdout("f6ad368298bd941e934a41f3babc827b2aa95a1d")
            .exit(0),
    ...
    def test_mode_full_copy(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="source", command=["git", "checkout", "-f", "FETCH_HEAD"]
            ).exit(0),
            ExpectCpdir(
                fromdir="source", todir="wkdir", log_environ=True, timeout=1200
            ).exit(0),
            ExpectShell(workdir="wkdir", command=["git", "rev-parse", "HEAD"])
            .stdout("f6ad368298bd941e934a41f3babc827b2aa95a1d")
            .exit(0),
    ...
    def test_mode_full_no_method(self):
        self.setup_step(
            self.stepClass(
                repourl="http://github.com/buildbot/buildbot.git", mode="full"
            )
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["git", "--version"])
    ...
    def test_mode_full_with_env(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", command=["git", "--version"], env={"abc": "123"}
            )
            .stdout("git version 1.7.5")
            .exit(0),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
    ...
    def test_mode_full_log_environ(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", command=["git", "--version"], log_environ=False
            )
            .stdout("git version 1.7.5")
            .exit(0),
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "rev-parse", "HEAD"], log_environ=False
            )
            .stdout("f6ad368298bd941e934a41f3babc827b2aa95a1d")
            .exit(0),
        )
    ...
    def setup_getDescription_test(
        self, setup_args, output_args, expect_head=True, codebase=None
    ):
    ...
    def test_getDescription_tags(self):
        self.setup_getDescription_test(
            setup_args={"tags": True}, output_args=["--tags"]
        )
        return self.run_step()
    ...
    def test_getDescription_abbrev(self):
        self.setup_getDescription_test(
            setup_args={"abbrev": 7}, output_args=["--abbrev=7"]
        )
        return self.run_step()

    def test_getDescription_abbrev_zero(self):
        self.setup_getDescription_test(
            setup_args={"abbrev": 0}, output_args=["--abbrev=0"]
        )
        return self.run_step()
    ...
    def test_getDescription_contains(self):
        self.setup_getDescription_test(
            setup_args={"contains": True}, output_args=["--contains"]
        )
        return self.run_step()
    ...
    def test_getDescription_exact_match_false(self):
        self.setup_getDescription_test(
            setup_args={"exact-match": False}, output_args=[]
        )
        return self.run_step()

    def test_getDescription_debug(self):
        self.setup_getDescription_test(
            setup_args={"debug": True}, output_args=["--debug"]
        )
        return self.run_step()
    ...
    def test_getDescription_long(self):
        self.setup_getDescription_test(
            setup_args={"long": True}, output_args=["--long"]
        )
    ...
    def test_getDescription_always(self):
        self.setup_getDescription_test(
            setup_args={"always": True}, output_args=["--always"]
        )
    ...
    def test_config_option(self):
        ...
        self.expect_commands(
            ...
            ExpectListdir(dir="wkdir").files([".git"]).exit(0),
            ExpectShell(
                workdir="wkdir", command=prefix + ["clean", "-f", "-f", "-d"]
            ).exit(0),
            ExpectShell(
                workdir="wkdir",
                command=prefix
            ...
            ExpectShell(
                workdir="wkdir", command=prefix + ["checkout", "-f", "FETCH_HEAD"]
            ).exit(0),
    ...
    def test_noGitCommandInstalled(self):
        @defer.inlineCallbacks
        def _dovccmd(
            command, abandonOnFailure=True, collectStdout=False, initialStdin=None
        ):
            """
    ...
    def test_config_get_description_not_dict_or_boolean(self):
        with self.assertRaisesConfigError(
            "Git: getDescription must be a boolean or a dict."
        ):
            self.stepClass(
                repourl="http://github.com/buildbot/buildbot.git",
                getDescription=["list"],
...
class TestGitPush(
    ...
    def test_push_simple(self):
        ...
        self.setup_step(
            self.stepClass(workdir="wkdir", repourl=url, branch="testbranch")
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["git", "--version"])
            .stdout("git version 1.7.5")
            .exit(0),
            ExpectShell(
                workdir="wkdir", command=["git", "push", url, "testbranch"]
            ).exit(0),
        )
        self.expect_outcome(result=SUCCESS)
        return self.run_step()
    ...
    def test_push_force(self):
        ...
        self.setup_step(
            self.stepClass(
                workdir="wkdir", repourl=url, branch="testbranch", force=True
            )
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["git", "--version"])
...
class TestGitTag(
    ...
    def test_tag_annotated(self):
        ...
        self.setup_step(
            self.stepClass(
                workdir="wkdir", tagName="myTag", annotated=True, messages=messages
            )
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["git", "--version"])
    ...
    def test_tag_force(self):
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectShell(
                workdir="wkdir", command=["git", "tag", "myTag", "--force"]
            ).exit(0),
        )
        self.expect_outcome(result=SUCCESS)
        return self.run_step()
    ...
    def test_config_annotated_message_not_list(self):
        with self.assertRaises(bbconfig.ConfigErrors):
            self.setup_step(
                self.stepClass(
                    workdir="wkdir", tagName="myTag", annotated=True, messages="msg"
                )
            )
...
class TestGitCommit(
    ...
    def test_add_fail(self):
        self.setup_step(
            self.stepClass(
                workdir="wkdir", paths=self.path_list, messages=self.message_list
            )
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["git", "--version"])
            ...
            .exit(0),
            ExpectShell(
                workdir="wkdir", command=["git", "add", "file1.txt", "file2.txt"]
            ).exit(1),
        )
        self.expect_outcome(result=FAILURE)
        return self.run_step()
    ...
    def test_commit_empty_ignore_witherror(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["git", "status", "--porcelain=v1"]
            ).exit(1),
        )
        self.expect_outcome(result=FAILURE)
        return self.run_step()
    ...
    def test_config_files_not_a_list(self):
        with self.assertRaisesConfigError("GitCommit: paths must be a list"):
            self.stepClass(
                workdir="wkdir", paths="test.txt", messages=self.message_list
            )
