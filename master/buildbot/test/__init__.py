...
warnings.filterwarnings(
    "ignore", r"The value of convert_charrefs will become True in 3.5"
)
...
warnings.filterwarnings(
    "ignore", r".*unclosed .*socket.*raddr=.*, 8200[^\d]", ResourceWarning
)
...
warnings.filterwarnings(
    "ignore", ".*Boto3 will no longer support Python .*", category=Warning
)

# autobahn is not updated for Twisted 22.04 and newer
warnings.filterwarnings(
    "ignore",
    "twisted.web.resource.NoResource was deprecated in",
    category=DeprecationWarning,
)
