...
class TestP4Poller(
    ...
    def do_test_poll_successful(self, **kwargs):
        ...
        self.expect_commands(
            ExpectMasterShell(
                ["p4", "changes", "-m", "1", "//depot/myproject/..."]
            ).stdout(first_p4changes),
            ExpectMasterShell(
                ["p4", "changes", "//depot/myproject/...@2,#head"]
    ...
    def test_poll_failed_describe(self):
        ...
        self.expect_commands(
            ExpectMasterShell(
                ["p4", "changes", "//depot/myproject/...@3,#head"]
            ).stdout(second_p4changes),
        )
    ...
    def test_acquire_ticket_auth(self):
        ...
        def spawnProcess(pp, cmd, argv, env):
            self.assertEqual(
                [cmd, argv], ["p4", [b"p4", b"-u", b"buildbot_user", b"login"]]
            )
    ...
    def test_poll_split_file(self):
        ...
        self.expect_commands(
            ExpectMasterShell(
                ["p4", "changes", "//depot/myproject/...@51,#head"]
