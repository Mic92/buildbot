...
class HgPoller(base.ReconfigurablePollingChangeSource, StateMixin):
    ...
    def checkConfig(
        ...
        name = self.build_name(
            name, repourl, bookmarks, self.build_branches(branch, branches)
        )
    ...
    def _getRevDetails(self, rev):
        ...
        if not self.usetimestamps:
            ...
            try:
                ...
                log.msg(
                    f"hgpoller: caught exception converting output {repr(date)} to timestamp"
                )
                raise
    ...
    def _initRepository(self):
        ...
        if rc != 0:
            ...
            raise EnvironmentError(
                f"{self}: repository init failed with exit code {rc}"
            )
    ...
    def _getHead(self, branch):
        ...
        if rc != 0:
            log.err(
                f"{self}: could not find revision {branch} in repository {self.repourl}"
            )
            return None
