...
class Latent(TimeoutableTestCase, RunFakeMasterTestCase):
    ...
    def test_refused_substantiations_get_requeued(self):
        ...
        self.assertEqual(
            set(brids), {req["buildrequestid"] for req in unclaimed_build_requests}
        )
    ...
    def test_substantiation_is_cancelled_by_build_stop(self):
        ...
        self.master.mq.produce(
            ("control", "builds", "1", "stop"), {"reason": "no reason"}
        )
    ...
    def test_reconfigservice_during_substantiation_clean_shutdown_after(
        ...
        self.assertEqual(
            registered_workers, [("local", "password_1"), ("local", "password_1")]
        )
    ...
    def test_substantiation_cancelled_by_insubstantiation_when_waiting_for_insubstantiation(
        ...
        yield self.create_build_request([builder_id])
        self.assertEqual(
            controller.worker.state, States.INSUBSTANTIATING_SUBSTANTIATING
        )
    ...
    def test_rejects_build_on_instance_with_different_type_timeout_zero(self):
        ...
        yield self.create_build_request(
            [builder_id], properties=Properties(worker_kind="a")
        )
        ...
        yield self.create_build_request(
            [builder_id], properties=Properties(worker_kind="b")
        )
        stepcontroller.finish_step(SUCCESS)
