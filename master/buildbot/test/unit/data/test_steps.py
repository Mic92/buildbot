...
class StepEndpoint(endpoint.EndpointMixin, unittest.TestCase):
    ...
    def test_get_existing_buildername_name(self):
        step = yield self.callGet(
            ("builders", "builder77", "builds", 7, "steps", "two")
        )
        self.validateData(step)
        self.assertEqual(step["stepid"], 71)
    ...
    def test_get_missing_buildername_builder_number(self):
        step = yield self.callGet(
            ("builders", "builder77_nope", "builds", 7, "steps", 1)
        )
    ...
    def setUp(self):
        ...
        self.db.insert_test_data(
            [
                ...
                fakedb.Step(
                    id=72, number=2, name="three", buildid=30, started_at=TIME4
                ),
                fakedb.Step(
                    id=73, number=0, name="otherbuild", buildid=31, started_at=TIME3
                ),
            ]
        )
...
class Step(TestReactorMixin, interfaces.InterfaceTests, unittest.TestCase):
    ...
    def test_fake_addStep(self):
        self.assertEqual(
            len((yield self.master.data.updates.addStep(buildid=10, name="ten"))), 3
        )
    ...
    def test_startStep(self):
        ...
        yield self.master.db.steps.addStep(
