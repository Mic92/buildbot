...
class GitPoller(base.ReconfigurablePollingChangeSource, StateMixin, GitMixin):
    ...
    def reconfigService(
        ...
        self.category = (
            ...
            else bytes2unicode(category, encoding=self.encoding)
        )
    ...
    def _get_commit_files(self, rev):
        ...
        def process(git_output):
            fileList = [
                decode_file(file)
                for file in [s for s in git_output.splitlines() if len(s)]
            ]
            return fileList
        ...
        if self.buildPushesWithNoCommits and not revList:
            ...
            if existingRev != newRev:
                ...
                if existingRev is None:
                    ...
                    log.msg(
                        f'gitpoller: rebuilding {newRev} for updated branch "{branch}"'
                    )
