...
class OAuth2Auth(
    TestReactorMixin, www.WwwTestMixin, ConfigErrorsMixin, unittest.TestCase
):
    ...
    def test_GoogleVerifyCode(self):
        ...
        self.googleAuth.get = mock.Mock(
            side_effect=[
                {"name": "foo bar", "email": "bar@foo", "picture": "http://pic"}
            ]
        )
    ...
    def test_GitHubAuthBadApiVersion(self):
        for bad_api_version in (2, 5, "a"):
            with self.assertRaisesConfigError(
                ...
                oauth2.GitHubAuth(
                    "ghclientID", "clientSECRET", apiVersion=bad_api_version
                )
    ...
    def test_GitHubAuthRaiseErrorWithApiV3AndGetTeamMembership(self):
        with self.assertRaisesConfigError(
            ...
            oauth2.GitHubAuth(
                "ghclientID", "clientSECRET", apiVersion=3, getTeamsMembership=True
            )
