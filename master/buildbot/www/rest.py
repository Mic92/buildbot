...
class V2RootResource(resource.Resource):
    ...
    def getEndpoint(self, request, method, params):
        ...
        yield self.master.www.assertUserAllowed(
            request, request_postpath, method, params
        )
    ...
    def handleErrors(self, writeError):
        try:
            ...
            writeError(
                repr(e), errcode=500, jsonrpccode=JSONRPC_CODES["internal_error"]
            )
            return
    ...
    def decodeJsonRPC2(self, request):
        ...
        if (
            ContentTypeParser(request.getHeader(b"content-type")).gettype()
            != "application/json"
        ):
            raise BadJsonRpc2(
        ...
        try:
            ...
            raise BadJsonRpc2(
                f"JSON parse error: {str(e)}", JSONRPC_CODES["parse_error"]
            ) from e
        ...
        def check(name, types, typename):
            if name not in data:
                raise BadJsonRpc2(
                    f"missing key '{name}'", JSONRPC_CODES["invalid_request"]
                )
            if not isinstance(data[name], types):
                raise BadJsonRpc2(
                    f"'{name}' must be {typename}", JSONRPC_CODES["invalid_request"]
                )
        ...
        if data["jsonrpc"] != "2.0":
            raise BadJsonRpc2(
                "only JSONRPC 2.0 is supported", JSONRPC_CODES["invalid_request"]
            )
    ...
    def renderJsonRpc(self, request):
        ...
        with self.handleErrors(writeError):
            ...
            data = json.dumps(
                jsonRpcReply, default=toJson, sort_keys=True, separators=(",", ":")
            )
    ...
    def encodeRaw(self, data, request, is_inline):
        request.setHeader(
            b"content-type", unicode2bytes(data["mime-type"]) + b"; charset=utf-8"
        )
        if not is_inline:
            request.setHeader(
    ...
    def renderRest(self, request):
        ...
        with self.handleErrors(writeError):
            ep, kwargs = yield self.getEndpoint(
                request, bytes2unicode(request.method), {}
            )
            ...
            if compact:
                data = json.dumps(
                    data, default=toJson, sort_keys=True, separators=(",", ":")
                )
            else:
