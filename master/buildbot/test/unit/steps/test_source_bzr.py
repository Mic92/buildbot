...
class TestBzr(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_mode_full(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir="wkdir", command=["bzr", "clean-tree", "--force"]).exit(
                0
            ),
    ...
    def test_mode_full_timeout(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", timeout=1, command=["bzr", "--version"]).exit(
                0
            ),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ExpectStat(file="wkdir/.bzr", log_environ=True).exit(0),
            ExpectShell(
                workdir="wkdir", timeout=1, command=["bzr", "clean-tree", "--force"]
            ).exit(0),
    ...
    def test_mode_full_revision(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir="wkdir", command=["bzr", "update", "-r", "3730"]).exit(
                0
            ),
            ExpectShell(
                workdir="wkdir",
    ...
    def test_mode_full_clean_revision(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir="wkdir", command=["bzr", "update", "-r", "2345"]).exit(
                0
            ),
            ExpectShell(
                workdir="wkdir",
    ...
    def test_mode_incremental_revision(self):
        ...
        self.expect_commands(
            ...
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ExpectStat(file="wkdir/.bzr", log_environ=True).exit(0),
            ExpectShell(workdir="wkdir", command=["bzr", "update", "-r", "9384"]).exit(
                0
            ),
            ExpectShell(
                workdir="wkdir",
