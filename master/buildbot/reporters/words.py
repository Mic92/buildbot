...
class Channel(service.AsyncService):
    ...
    def validate_notification_event(self, event):
        if not re.compile(
            ...
            raise UsageError(
                "Try '" + self.bot.commandPrefix + "notify on|off _EVENT_'."
            )
    ...
    def buildStarted(self, build):
        ...
        if self.bot.tags is not None and not self.builderMatchesAnyTag(
            builder.get("tags", [])
        ):
        ...
        if self.useRevisions:
            ...
            url = utils.getURLForBuild(
                self.master, builder["builderid"], build["number"]
            )
        ...
        if result in self.bot.results_severity and (
            ...
            if prev_build:
                ...
                if prev_result in self.bot.results_severity:
                    ...
                    if (
                        self.notify_for("better")
                        and result_severity < prev_result_severity
                    ):
                        return True
                    if (
                        self.notify_for("worse")
                        and result_severity > prev_result_severity
                    ):
                        return True
...
class Contact:
    ...
    command_STATUS.usage = (
        "status [_which_] - list status of a builder (or all builders)"
    )
    ...
    def command_NOTIFY(self, args, **kwargs):
        ...
        if not args:
            raise UsageError(
                "Try '" + self.bot.commandPrefix + "notify on|off|list [_EVENT_]'."
            )
        action = args.pop(0)
        events = args
    ...
    def command_STOP(self, args, **kwargs):
        ...
        if len(args) < 3 or args[0] != "build":
            raise UsageError(
                "Try '" + self.bot.commandPrefix + "stop build _which_ _reason_'."
            )
        which = args[1]
        reason = " ".join(args[2:])
        ...
        for bdict in builds:
            ...
            if self.bot.useRevisions:
                ...
                response = (
                    f"Build containing revision(s) {','.join(revisions)} interrupted"
                )
            else:
    ...
    def command_COMMANDS(self, args, **kwargs):
        ...
        commands = self.build_commands()
        str = "Buildbot commands: " + ", ".join(
            self.bot.commandPrefix + c for c in commands
        )
        self.send(str)
    ...
    def command_SHUTDOWN(self, args, **kwargs):
        ...
        if args not in ("check", "start", "stop", "now"):
            raise UsageError(
                "Try '" + self.bot.commandPrefix + "shutdown check|start|stop|now'."
            )
...
class StatusBot(service.AsyncMultiService):
    ...
    def loadState(self):
        yield self._load_channels_state(
            "notify_events", lambda c, e: c.add_notification_events(e)
        )
        yield self._load_channels_state(
    ...
    def log_err(self, error=None, why=None, source=None):
        log.callWithContext(
            {"system": (self._get_log_system(source))}, log.err, error, why
        )
...
class WebhookResource(resource.Resource, service.AsyncService):
    ...
    def render_POST(self, request):
        ...
        def err(error):
            try:
                self.parent.log_err(
                    error, "processing telegram request", self.__class__.__name__
                )
            except AttributeError:
