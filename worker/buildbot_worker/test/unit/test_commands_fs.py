...
class TestCopyDirectory(CommandTestMixin, unittest.TestCase):
    ...
    def test_simple(self):
        ...
        self.make_command(
            fs.CopyDirectory, {"from_path": from_path, "to_path": to_path}, True
        )
        yield self.run_command()
...
class TestMakeDirectory(CommandTestMixin, unittest.TestCase):
    ...
    def test_error_existing_file(self):
        ...
        self.assertIn(
            ("rc", errno.EEXIST), self.get_updates(), self.protocol_command.show()
        )
...
class TestStatFile(CommandTestMixin, unittest.TestCase):
    ...
    def test_non_existent(self):
        ...
        self.assertIn(
            ("rc", errno.ENOENT), self.get_updates(), self.protocol_command.show()
        )
...
class TestGlobPath(CommandTestMixin, unittest.TestCase):
    ...
    def test_non_existent(self):
        self.make_command(
            fs.GlobPath, {"path": os.path.join(self.basedir, "no-*-file")}, True
        )
        yield self.run_command()
    ...
    def test_directory(self):
        self.make_command(
            fs.GlobPath, {"path": os.path.join(self.basedir, "[wxyz]or?d*")}, True
        )
        yield self.run_command()

        self.assertEqual(
            self.get_updates()[0][1], [os.path.join(self.basedir, "workdir")]
        )
        self.assertIn(("rc", 0), self.get_updates(), self.protocol_command.show())
    ...
    def test_file(self):
        self.make_command(
            fs.GlobPath, {"path": os.path.join(self.basedir, "t*-file")}, True
        )
        with open(os.path.join(self.basedir, "test-file"), "w"):
            pass
        ...
        yield self.run_command()
        self.assertEqual(
            self.get_updates()[0][1], [os.path.join(self.basedir, "test-file")]
        )
        self.assertIn(("rc", 0), self.get_updates(), self.protocol_command.show())
    ...
    def test_recursive(self):
        self.make_command(
            fs.GlobPath, {"path": os.path.join(self.basedir, "**/*.txt")}, True
        )
