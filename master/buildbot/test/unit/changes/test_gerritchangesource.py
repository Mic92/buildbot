...
class TestGerritChangeSource(
    ...
    def assert_changes(self, expected_changes, ignore_keys):
        self.assertEqual(
            len(self.master.data.updates.changesAdded), len(expected_changes)
        )
    ...
    def test_lineReceived_patchset_created(self):
        ...
        self.assert_changes(
            [self.expected_change_patchset_created], ignore_keys=["properties"]
        )
    ...
    comment_added_event = {
        ...
        "approvals": [
            {"type": "Code-Review", "description": "Code-Review", "value": "0"}
        ],
        "comment": "Patch Set 1:\n\ntest comment",
        "patchSet": {
            "number": 1,
    ...
    def test_lineReceived_comment_added(self):
        ...
        self.assert_changes(
            [self.expected_change_comment_added], ignore_keys=["properties"]
        )
...
class TestGerritChangeFilter(unittest.TestCase):
    def test_event_type(self):
        ...
        ch = Change(
            **TestGerritChangeSource.expected_change_patchset_created, properties=props
        )
    ...
    def test_event_type_re(self):
        ...
        self.assertFalse(
            f.filter_change(Change(properties=self.create_props("br", "ref-updated")))
        )

    def test_event_type_fn(self):
        f = gerritchangesource.GerritChangeFilter(
            eventtype_fn=lambda t: t == "patchset-created"
        )
        self.assertTrue(
            f.filter_change(
                Change(properties=self.create_props("br", "patchset-created"))
