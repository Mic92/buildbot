...
class TestEC2LatentWorker(unittest.TestCase):
    ...
    def _patch_moto_describe_spot_price_history(self, bs, instance_type, price):
        def fake_describe_price(*args, **kwargs):
            return {
                "SpotPriceHistory": [
                    {"InstanceType": instance_type, "SpotPrice": price}
                ]
            }

        self.patch(
            bs.ec2.meta.client, "describe_spot_price_history", fake_describe_price
        )
    ...
    def test_start_instance_attach_volume(self):
        ...
        sdz = [
            bm
            for bm in instance.block_device_mappings
            if bm["DeviceName"] == "/dev/sdz"
        ][0]
