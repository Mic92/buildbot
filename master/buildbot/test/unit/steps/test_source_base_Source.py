...
class TestSource(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_start_alwaysUseLatest_False(self):
        ...
        self.assertEqual(
            step.run_vc.mock.call_args, (("other-branch", "revision", "patch"), {})
        )
...
class TestSourceAttrGroup(
    sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase
):
    def setUp(self):
        self.setup_test_reactor()
        return self.setup_test_build_step()
    ...
    def test_attrgroup_getattr(self):
        ...
        self.assertEqual(
            step._getAttrGroupMember("mode", "incremental"), step.mode_incremental
        )
    ...
    def test_attrgroup_listattr(self):
        step = AttrGroup()
        self.assertEqual(
            sorted(step._listAttrGroupMembers("mode")), ["full", "incremental"]
        )
