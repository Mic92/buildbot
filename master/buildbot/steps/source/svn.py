...
class SVN(Source):
    ...
    def _sourcedirIsUpdatable(self):
        ...
        try:
            ...
            extractedurl = stdout_xml.getElementsByTagName("url")[
                0
    ...
    def parseGotRevision(self):
        ...
        if self.preferLastChangedRev:
            try:
                revision = (
                    stdout_xml.getElementsByTagName("commit")[0]
                    .attributes["revision"]
                    .value
                )
        ...
        if revision is None:
            try:
                revision = (
                    stdout_xml.getElementsByTagName("entry")[0]
                    .attributes["revision"]
                    .value
                )
            except (KeyError, IndexError) as e:
                msg = (
                    "SVN.parseGotRevision unable to detect revision in"
                    " output of svn info"
                )
                log.msg(msg)
                raise buildstep.BuildStepFailed() from e
    ...
    def removeFiles(self, files):
        for filename in files:
            res = yield self.runRmdir(
                filename, abandonOnFailure=False, timeout=self.timeout
            )
            if res:
                return res
        return 0
    ...
    def _checkout(self):
        ...
        if self.retry:
            ...
            if repeats > 0:
                log.msg(
                    f"Checkout failed, trying {repeats} more times after {delay} seconds"
                )
                self.retry = (delay, repeats - 1)
                df = defer.Deferred()
                df.addCallback(
                    lambda _: self.runRmdir(self.workdir, timeout=self.timeout)
                )
