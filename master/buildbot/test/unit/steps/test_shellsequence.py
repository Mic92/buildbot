...
class TestOneShellCommand(
    ...
    def test_shell_arg_error_logfile_and_logname(self):
        with assertProducesWarnings(
            ...
            with self.assertRaisesConfigError(
                ...
                shellsequence.ShellArg(
                    command="command", logname="logname", logfile="logfile"
                )
    ...
    def testMultipleCommandsAreRun(self):
        ...
        self.setup_step(
            shellsequence.ShellSequence(commands=[arg1, arg2], workdir="build")
        )
        self.expect_commands(
    ...
    def testSkipWorks(self):
        ...
        self.setup_step(
            shellsequence.ShellSequence(commands=[arg1, arg2, arg3], workdir="build")
        )
        self.expect_commands(
            ExpectShell(workdir="build", command="make p1").exit(0),
            ExpectShell(workdir="build", command="deploy p1").exit(0),
    ...
    def testWarningWins(self):
        arg1 = shellsequence.ShellArg(
            command="make p1", warnOnFailure=True, flunkOnFailure=False
        )
        arg2 = shellsequence.ShellArg(command="deploy p1")
        self.setup_step(
            shellsequence.ShellSequence(commands=[arg1, arg2], workdir="build")
