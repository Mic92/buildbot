...
class BBService(win32serviceutil.ServiceFramework):
    ...
    _svc_description_ = (
        "Manages local buildbot workers and masters - see https://buildbot.net"
    )
    ...
    def _checkConfig(self):
        ...
        if not self.dirs:
            self.error(
                "No valid buildbot directories were specified.\n"
                "Stopping the service."
            )
            return False
        if save_dirs:
    ...
    def SvcDoRun(self):
        ...
        for bbdir in self.dirs:
            ...
            cmd = "{} --spawn {} start --nodaemon {}".format(
        ...
        while child_infos:
            ...
            if not child_infos:
                self.warning(
                    "All BuildBot child processes have "
        ...
        for bbdir, h, t, output in child_infos:
            ...
            if win32process.GetExitCodeProcess(h) == win32con.STILL_ACTIVE:
                self.warning(
                    "BuildBot process at {0!r} failed to terminate - "
    ...
    def dup(self, pipe):
        ...
        dup = win32api.DuplicateHandle(
            pid, pipe, pid, 0, 0, win32con.DUPLICATE_SAME_ACCESS
        )
        pipe.Close()
        return dup
