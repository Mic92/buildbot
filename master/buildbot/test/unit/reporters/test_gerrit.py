...
def sampleStartCBDeferred(builderName, build, arg):
    result = yield makeReviewResult(
        str({"name": builderName}), (GERRIT_LABEL_REVIEWED, 0)
    )
    return result
...
def sampleSummaryCBDeferred(buildInfoList, results, master, arg):
    ...
    result = yield makeReviewResult(
        str(buildInfoList), (GERRIT_LABEL_VERIFIED, verified)
    )
    return result
...
class TestGerritStatusPush(TestReactorMixin, unittest.TestCase, ReporterTestMixin):
    ...
    def check_summary_build_deferred(
        ...
        msg = yield self.run_fake_summary_build(
            gsp, buildResults, finalResult, resultText
        )
    ...
    @defer.inlineCallbacks
    def check_summary_build_legacy(
        self, buildResults, finalResult, resultText, verifiedScore
    ):
    ...
    def test_buildsetComplete_filtered_builder(self):
        gsp = yield self.setupGerritStatusPush(summaryCB=sampleSummaryCB)
        gsp.builders = ["foo"]
        yield self.run_fake_summary_build(
            gsp, [FAILURE, FAILURE], FAILURE, ["failed", "failed"]
        )
    ...
    def test_callWithVersion_bytes_output(self):
        ...
        gsp.callWithVersion(
            lambda: self.assertEqual(gsp.gerrit_version, parse_version("2.14"))
        )
