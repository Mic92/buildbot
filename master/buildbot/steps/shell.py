...
class WarningCountingShellCommand(
    buildstep.ShellMixin, CompositeStepMixin, buildstep.BuildStep
):
    renderables = [
        "suppressionFile",
        "suppressionList",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_exact_step_class(WarningCountingShellCommand) and not kwargs.get(
            "command"
        ):
    ...
    def setup_suppression(self):
        ...
        if self.suppressionFile is not None:
            data = yield self.getFileContentFromWorker(
