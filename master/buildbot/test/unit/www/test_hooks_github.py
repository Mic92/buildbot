...
def _prepare_github_change_hook(testcase, **params):
    return ChangeHookResource(
        dialects={"github": params}, master=fakeMasterForHooks(testcase)
    )
...
class TestChangeHookConfiguredWithGitChange(unittest.TestCase, TestReactorMixin):
    ...
    def test_git_with_pull_encoded(self):
        commit_endpoint = (
            "/repos/defunkt/github/commits/05c588ba8cd510ecbe112d020f215facb17817a7"
        )
        files_endpoint = "/repos/defunkt/github/pulls/50/files"
        self._http.expect(
            "get", commit_endpoint, content_json=gitJsonPayloadNotFound, code=404
        )
        self._http.expect(
            "get", files_endpoint, content_json=gitJsonPayloadNotFound, code=404
        )
...
class TestChangeHookConfiguredWithAuthAndCustomSkips(
    unittest.TestCase, TestReactorMixin
):
    @defer.inlineCallbacks
    def setUp(self):
...
class TestChangeHookConfiguredWithCustomApiRootWithAuth(
    unittest.TestCase, TestReactorMixin
):
    @defer.inlineCallbacks
    def setUp(self):
...
class TestChangeHookConfiguredWithCustomEventHandler(
    unittest.TestCase, TestReactorMixin
):
    def setUp(self):
        self.setup_test_reactor()
