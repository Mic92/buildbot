...
class UpgradeTestMixin(db.RealDatabaseMixin, TestReactorMixin):
    ...
    def assertModelMatches(self):
        def comp(engine):
            ...
            with engine.connect() as conn:
                diff = compare_metadata(
                    MigrationContext.configure(conn), self.db.model.metadata
                )
            ...
            for tbl in self.db.model.metadata.sorted_tables:
                ...
                if engine.dialect.name == "mysql":
                    implied = [
                        idx
                        for (tname, idx) in self.db.model.implied_indexes
                        if tname == tbl.name
                    ]
        ...
        try:
            ...
            raise unittest.SkipTest(
                "model comparison skipped: bugs in schema "
    ...
    def gotError(self, e):
        if isinstance(e, (sqlite3.DatabaseError, DatabaseError)):
            if "file is encrypted or is not a database" in str(e):
                ...
                raise unittest.SkipTest(
                    f"sqlite dump not readable on this machine {str(e)}"
                )
...
class UpgradeTestV2_10_5(UpgradeTestMixin, unittest.TestCase):
    ...
    def test_got_invalid_sqlite_file2(self):
        def upgrade():
            return defer.fail(
                DatabaseError("file is encrypted or is not a database", None, None)
            )
