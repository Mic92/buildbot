...
class TelegramChannel(Channel):
    def __init__(self, bot, channel):
        assert isinstance(
            channel, dict
        ), "channel must be a dict provided by Telegram API"
    ...
    def list_notified_events(self):
        if self.notify_events:
            notified_events = "\n".join(
                sorted(f"🔔 **{n}**" for n in self.notify_events)
            )
            yield self.send(
                f"The following events are being notified:\n{notified_events}"
            )
        else:
...
class TelegramContact(Contact):
    ...
    def command_GETID(self, args, **kwargs):
        ...
        if self.is_private_chat:
            ...
            self.send(
                f'This {self.channel.chat_info.get("type", "group")} ID is `{self.chat_id}`.'
            )

    command_GETID.usage = (
        "getid - get user and chat ID that can be put in the master "
        "configuration file"
    )
    ...
    def command_LIST(self, args, **kwargs):
        ...
        if not args:
            ...
            self.send(
                "What do you want to list?", reply_markup={"inline_keyboard": keyboard}
            )
            return
            ...
            if all:
                ...
                if num > 50:
                    keyboard = [
                        ...
                        [
                            self.query_button(
                                "✅ No, just show last 50", "/list 50 changes"
                            )
                        ],
                    ]
                    self.send(
                ...
                changes = yield self.master.data.get(
                    ("changes",), order=["-changeid"], limit=num
                )
                self.bot.delete_message(self.channel.id, wait_message["message_id"])
    ...
    def command_WATCH(self, args, **kwargs):
        if args:
            ...
            if builders:
                keyboard = [
                    [self.query_button("🔎 " + b, f"/watch {b}")] for b in builders
                ]
                self.send(
    ...
    def command_NOTIFY(self, args, tquery=None, **kwargs):
        ...
        if tquery:
            self.bot.edit_keyboard(
                self.chat_id, tquery["message"]["message_id"], keyboard
            )
        else:
            self.send(
    ...
    def command_STOP(self, args, **kwargs):
        ...
        if not argv:
            ...
            if builders:
                keyboard = [
                    [self.query_button("🚫 " + b, f"/stop build {b}")] for b in builders
                ]
                self.send(
    ...
    def command_SHUTDOWN(self, args, **kwargs):
        ...
        if self.master.botmaster.shuttingDown:
            ...
            text = ""
        self.send(
            text + "What do you want to do?", reply_markup={"inline_keyboard": keyboard}
        )
        return None
            ...
            try:
                if missing_params:
                    ...
                    for scheduler in self.master.allSchedulers():
                        if scheduler.name == sched and isinstance(
                ...
                raise UsageError(
                    f"Try '/force' and follow the instructions ({e})"
                ) from e
        ...
        if task == "config":
            ...
            if not missing_params:
                keyboard.append(
                    [
                        self.query_button(
                            "🚀 Start Build", f"/force {sched} build {bldr} {args}"
                        )
                    ],
                )
            ...
            args = " ".join(
                shlex.quote(f"{p[0]}={p[1]}") for p in params.items() if p[0] != what
            )
...
class TelegramStatusBot(StatusBot):
    ...
    def getContact(self, user, channel):
        ...
        try:
            ...
            contact = self.contactClass(
                user=user, channel=self.getChannel(channel, valid)
            )
            if valid:
...
class TelegramWebhookBot(TelegramStatusBot):
    ...
    def set_webhook(self, url, certificate=None):
        if not certificate:
            ...
            yield self.post(
                "/setWebhook", data={"url": url}, files={"certificate": certificate}
            )
    ...
    def reconfigService(
        ...
        if bot_username is not None:
            ...
            if self.bot.nickname is None:
                raise RuntimeError(
                    "No bot username specified and I cannot get it from Telegram"
                )
