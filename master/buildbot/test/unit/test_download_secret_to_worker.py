...
class TestRemoveWorkerFileSecretCommand30(
    TestBuildStepMixin, TestReactorMixin, unittest.TestCase
):
    def setUp(self):
        self.setup_test_reactor()
...
class TestRemoveFileSecretToWorkerCommand(
    ...
    def testBasic(self):
        ...
        self.expect_commands(
            ExpectRmfile(
                path=os.path.join(self.temp_path, "pathA"), log_environ=False
            ).exit(0),
            ExpectRmfile(
                path=os.path.join(self.temp_path, "pathB"), log_environ=False
            ).exit(0),
        )
