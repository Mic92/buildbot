...
class BaseParameter:
    ...
    def __init__(self, name, label=None, tablabel=None, regex=None, **kw):
        ...
        if name in ["owner", "builderNames", "builderid"]:
            config.error(
                f"{name} cannot be used as a parameter name, because it is reserved"
            )
        self.name = name
    ...
    def getFromKwargs(self, kwargs):
            ...
            for arg in args:
                if not self.regex.match(arg):
                    raise ValidationError(
                        f"{self.label}:'{arg}' does not match pattern "
        ...
        if arg is None:
            raise ValidationError(
                f"need {self.fullName}: no default provided by config"
            )
        return arg
...
class ChoiceStringParameter(BaseParameter):
    ...
    def parse_from_arg(self, s):
        if self.strict and s not in self.choices:
            raise ValidationError(
                f"'{s}' does not belong to list of available choices "
                f"'{self.choices}'"
            )
        return s
...
class CodebaseParameter(NestedParameter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        fields = [
            val for k, val in sorted(fields_dict.items(), key=lambda x: x[0]) if val
        ]
        ...
        super().__init__(
            name=name, label=label, codebase=codebase, fields=fields, **kwargs
        )
...
class ForceScheduler(base.BaseScheduler):
    ...
    def __init__(
        ...
        if self.checkIfType(reason, BaseParameter):
            ...
            config.error(
                f"ForceScheduler '{name}': reason must be a StringParameter: "
                f"{repr(reason)}"
            )
        ...
        for codebase in codebases:
            ...
            codebase_dict[codebase.codebase] = {
                "branch": "",
                "repository": "",
                "revision": "",
            }
        ...
        if self.checkIfType(priority, IntParameter):
            ...
            config.error(
                f"ForceScheduler '{name}': priority must be a IntParameter: "
                f"{repr(priority)}"
            )
    ...
    def force(self, owner, builderNames=None, builderid=None, **kwargs):
        ...
        kwargs = dict(
            (k, [v]) if not isinstance(v, list) else (k, v) for k, v in kwargs.items()
        )
