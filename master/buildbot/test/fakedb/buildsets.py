...
class BuildsetProperty(Row):
    ...
    def __init__(
        self, buildsetid=None, property_name="prop", property_value='[22, "fakedb"]'
    ):
        super().__init__(
...
class FakeBuildsetsComponent(FakeDBComponent):
    ...
    def insert_test_data(self, rows):
        ...
        for row in rows:
            if isinstance(row, BuildsetSourceStamp):
                assert row.buildsetid in self.buildsets
                self.buildset_sourcestamps.setdefault(row.buildsetid, []).append(
                    row.sourcestampid
                )
    ...
    def addBuildset(
        ...
        assert isinstance(
            waited_for, bool
        ), f"waited_for should be boolean: {repr(waited_for)}"
    ...
    def getRecentBuildsets(
        self, count=None, branch=None, repository=None, complete=None
    ):
        if not count:
            return []
        rv = []
