...
class MailNotifier(ReporterBase):
    ...
    def patch_to_attachment(self, patch, index):
        ...
        a.add_header(
            "Content-Disposition", "attachment", filename="source patch " + str(index)
        )
        return a

    @defer.inlineCallbacks
    def createEmail(
        self, msgdict, title, results, builds=None, patches=None, logs=None
    ):
        text = msgdict["body"]
        type = msgdict["type"]
    ...
    def processRecipients(self, blamelist, m):
        ...
        m["To"] = ", ".join(
            [self.formatAddress(addr) for addr in sorted(to_recipients)]
        )
        if cc_recipients:
            m["CC"] = ", ".join(
                [self.formatAddress(addr) for addr in sorted(cc_recipients)]
            )
