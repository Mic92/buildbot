...
class HashiCorpVaultSecretProvider(SecretProviderBase):
    ...
    def checkConfig(
        self, vaultServer=None, vaultToken=None, secretsmount=None, apiVersion=1
    ):
        warn_deprecated(
            "3.4.0",
        ...
        if not isinstance(vaultServer, str):
            config.error(
                f"vaultServer must be a string while it is {type(vaultServer)}"
            )
    ...
    def get(self, entry):
        ...
        try:
            ...
            raise KeyError(
                f"The secret {entry} does not exist in Vault provider: {e}"
            ) from e
