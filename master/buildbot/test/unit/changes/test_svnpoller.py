...
class TestSVNPoller(
    ...
    def do_test_get_prefix(self, base, output, expected):
        ...
        self.expect_commands(
            ExpectMasterShell(
                ["svn", "info", "--xml", "--non-interactive", base]
            ).stdout(output)
        )
    ...
    def test_poll_no_password(self):
        s = yield self.attachSVNPoller(
            sample_base, split_file=split_file, svnuser="dustin"
        )
...
class TestSplitFile(unittest.TestCase):
    ...
    def test_split_file_branches_trunk_subdir(self):
        self.assertEqual(
            svnpoller.split_file_branches("trunk/subdir/"), (None, "subdir/")
        )
    ...
    def test_split_file_branches_branch(self):
        self.assertEqual(
            svnpoller.split_file_branches("branches/1.5.x/"), ("branches/1.5.x", "")
        )
