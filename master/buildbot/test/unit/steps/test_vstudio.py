...
class TestAddEnvPath(unittest.TestCase):
    ...
    def test_existing(self):
        self.do_test(
            {"PATH": "/bin"}, "PATH", r"C:\NOTHING", {"PATH": r"/bin;C:\NOTHING;"}
        )
...
class VisualStudio(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_simple(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["command", "here"]).exit(0)
        )
    ...
    def test_evaluate_result_failure(self):
        self.setup_step(VCx())
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["command", "here"]).exit(1)
        )
        self.expect_outcome(
            result=FAILURE, state_string="compile 0 projects 0 files (failure)"
        )
        return self.run_step()
...
class TestVC6(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_clean(self):
        self.setup_step(
            vstudio.VC6(projectfile="pf", config="cfg", project="pj", mode="clean")
        )
        self.expect_commands(
            ExpectShell(
                workdir="wkdir",
...
class TestVC7(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_clean(self):
        self.setup_step(
            vstudio.VC7(projectfile="pf", config="cfg", project="pj", mode="clean")
        )
        self.expect_commands(
            ExpectShell(
                workdir="wkdir",
...
class TestVC8(
    VC8ExpectedEnvMixin, TestBuildStepMixin, TestReactorMixin, unittest.TestCase
):
    def setUp(self):
        self.setup_test_reactor()
        return self.setup_test_build_step()
    ...
    def test_args(self):
        self.setup_step(
            vstudio.VC8(projectfile="pf", config="cfg", project="pj", arch="arch")
        )
        self.expect_commands(
            ExpectShell(
                workdir="wkdir",
    ...
    def test_args_x64(self):
        self.setup_step(
            vstudio.VC8(projectfile="pf", config="cfg", project="pj", arch="x64")
        )
        self.expect_commands(
            ExpectShell(
                workdir="wkdir",
                command=["devenv.com", "pf", "/Rebuild", "cfg", "/Project", "pj"],
                env=self.getExpectedEnv(
                    r"C:\Program Files\Microsoft Visual Studio 8", x64=True
                ),
            ).exit(0)
        )
        self.expect_outcome(result=SUCCESS, state_string="compile 0 projects 0 files")
    ...
    def test_clean(self):
        self.setup_step(
            vstudio.VC8(projectfile="pf", config="cfg", project="pj", mode="clean")
        )
        self.expect_commands(
            ExpectShell(
                workdir="wkdir",
...
class TestVCExpress9(
    ...
    def test_clean(self):
        self.setup_step(
            vstudio.VCExpress9(
                projectfile="pf", config="cfg", project="pj", mode="clean"
            )
        )
        self.expect_commands(
            ExpectShell(
    ...
    def test_mode_build_env(self):
        ...
        self.expect_commands(
            ExpectShell(
                ...
                env=self.getExpectedEnv(
                    r"C:\Program Files\Microsoft Visual Studio 8", i="i"
                ),
            ).exit(0)
        )
        self.expect_outcome(result=SUCCESS, state_string="compile 0 projects 0 files")
...
class TestMsBuild(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_no_platform(self):
        ...
        self.expect_outcome(
            result=results.EXCEPTION, state_string="built pj for cfg|None"
        )
        yield self.run_step()
