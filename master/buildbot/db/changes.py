...
class ChangesConnectorComponent(base.DBConnectorComponent):
    ...
    def addChange(
        ...
        if is_dir is not None:
            log.msg(
                "WARNING: change source is providing deprecated "
        ...
        for pv in properties.values():
            assert pv[1] == "Change", (
                "properties must be qualified with" "source 'Change'"
            )
        ...
        parent_changeids = yield self.getParentChangeIds(
            branch, repository, project, codebase
        )
        ...
        def thd(conn):
            ...
            if files:
                ...
                conn.execute(
                    tbl.insert(), [{"changeid": changeid, "filename": f} for f in files]
                )
            if properties:
    ...
    def getChangesForBuild(self, buildid):
        ...
        for cb, change in fromChanges.items():
            ...
            if change and change["changeid"] != toCbChange.get("changeid"):
                ...
                while (
                    toCbChange.get("changeid") not in change["parent_changeids"]
                ) and change["parent_changeids"]:
                    # For the moment, a Change only have 1 parent.
                    change = yield self.master.db.changes.getChange(
                        change["parent_changeids"][0]
                    )
