...
class BuilderConfigTests(ConfigErrorsMixin, unittest.TestCase):
    ...
    def test_tags_must_be_list_of_str(self):
        with self.assertRaisesConfigError(
            ...
            BuilderConfig(
                tags=["abc", 13], name="a", workernames=["a"], factory=self.factory
            )
    ...
    def test_init_next_worker_keyword(self):
        f = lambda: None
        cfg = BuilderConfig(
            name="a b c", workername="a", factory=self.factory, nextWorker=f
        )
