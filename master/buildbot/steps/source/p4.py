...
class P4(Source):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            self.mode not in self.possible_modes
            and not interfaces.IRenderable.providedBy(self.mode)
        ):
            config.error(
        ...
        if (
            not interfaces.IRenderable.providedBy(p4base)
            and p4base
            and not p4base.startswith("/")
        ):
        ...
        if (
            not interfaces.IRenderable.providedBy(p4branch)
            and p4branch
            and p4branch.endswith("/")
        ):
            config.error(
                f"p4branch should not end with a trailing / [p4branch = {p4branch}]"
            )
        ...
        if stream:
            ...
            if not p4base or not p4branch:
                config.error(
                    "You must specify both p4base and p4branch when using stream"
                )
    ...
    def run_vc(self, branch, revision, patch):
        ...
        if self.p4passwd is not None:
            if not self.workerVersionIsOlderThan("shell", "2.16"):
                ...
                log.msg(
                    "Worker does not understand obfuscation; "
    ...
    def mode_full(self):
        ...
        if self.revision:
            ...
            yield self._dovccmd(
                ["sync", f"//{self.p4client}/...@{int(self.revision)}"],
                collectStdout=True,
            )
        else:
            if self.debug:
                log.msg(
                    "P4: full() sync command based on :client:%s no revision",
                    self.p4client,
                )
    ...
    def _createClientSpec(self):
        ...
        root = self.build.path_module.normpath(
        ...
        if self.stream:
            ...
            if self.p4viewspec:
                ...
                for k, v in self.p4viewspec:
                    ...
                    client_spec += (
                        f"\t{qa}{k}{suffix}{qa} {qb}//{self.p4client}/{v}{suffix}{qb}\n"
                    )
            else:
                ...
                if self.p4extra_views:
                    for k, v in self.p4extra_views:
                        ...
                        client_spec += (
                            f"\t{qa}{k}/...{qa} {qb}//{self.p4client}/{v}/...{qb}\n"
                        )
    ...
    def get_sync_revision(self, revision=None):
        ...
        if self.debug:
            log.msg(
                "P4: get_sync_revision() retrieve client actual revision at %s",
                revision,
            )

        changes_command_args = [
            "-ztag",
            "changes",
            "-m1",
            f"//{self.p4client}/...{revision}",
        ]
