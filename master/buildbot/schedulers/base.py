...
class BaseScheduler(ClusteredBuildbotService, StateMixin):
    ...
    def __init__(
        self, name, builderNames, properties=None, codebases=None, priority=None
    ):
        ...
        if codebases is None:
            ...
            for codebase, attrs in codebases.items():
                if not isinstance(attrs, dict):
                    ...
                    if unk:
                        config.error(
                            f"Unknown codebase keys {', '.join(unk)} "
    ...
    def __repr__(self):
        ...
        """
        return f"<{self.__class__.__name__}({self.name}, {self.builderNames}, enabled={self.enabled})>"
    ...
    def startConsumingChanges(
        ...
        self._change_consumer = yield self.master.mq.startConsuming(
            lambda k, m: self._changeCallback(
                k, m, fileIsImportant, change_filter, onlyImportant
            ),
            ("changes", None, "new"),
        )
    ...
    def _changeCallback(self, key, msg, fileIsImportant, change_filter, onlyImportant):
        ...
        if change_filter:
            ...
            change_filter_may_be_broken = (
                change.category == "ref-updated"
                and not change.branch.startswith("refs/")
            )
            ...
            if change_filter_may_be_broken:
                ...
                if (
                    old_filter_result != new_filter_result
                    and has_deprecated_ref_branch_filter()
                ):
                    warn_deprecated(
                        "3.5.0",
    ...
    def addBuildsetForChanges(
        ...
        **kw,
        ...
        def get_last_change_for_codebase(codebase):
            return max(
                changesByCodebase[codebase], key=lambda change: change["changeid"]
            )
