...
class TestMakeTAC(TestDefaultOptionsMixin, unittest.TestCase):
    ...
    def assert_tac_file_contents(self, tac_contents, expected_args, relocate=None):
        ...
        self.assertEqual(
            worker_mock.method_calls, [mock.call.setServiceParent(application_mock)]
        )

        # .tac file must define global variable "application", instance of
        # Application
        self.assertTrue(
            "application" in globals_dict,
    ...
    def test_relocatable_true(self):
        ...
        self.assert_tac_file_contents(
            tac_contents, options, relocate=options["basedir"]
        )
...
class TestMakeBuildbotTac(
    misc.StdoutAssertionsMixin, misc.FileIOMixin, unittest.TestCase
):
...
class TestCreateWorker(
    misc.StdoutAssertionsMixin, TestDefaultOptionsMixin, unittest.TestCase
):
    ...
    def testCreateError(self):
        ...
        self.assertEqual(
            create_worker.createWorker(self.options), 1, "unexpected exit code"
        )
    ...
    def testMinArgs(self):
        ...
        self.assertEqual(
            create_worker.createWorker(self.options), 0, "unexpected exit code"
        )
    ...
    def testQuiet(self):
        ...
        self.assertMakeFunctionsCalls(
            options["basedir"], expected_tac_contents, options["quiet"]
        )
