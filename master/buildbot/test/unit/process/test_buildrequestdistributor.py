...
class Test(TestBRDBase):
    ...
    def test_maybeStartBuildsOn_exception(self):
        ...
        def _maybeStartBuildsOnBuilder(n):
            ...
            self.reactor.callLater(
                0, d.errback, failure.Failure(RuntimeError("oh noes"))
            )
            return d
    ...
    def test_stopService(self):
        ...
        def maybeStartBuildsOnBuilder(bldr):
            ...
            stop_d.addCallback(
                lambda _: self.maybeStartBuildsOnBuilder_calls.append("(stopped)")
            )

            d.addCallback(
                lambda _: self.maybeStartBuildsOnBuilder_calls.append("finished")
            )
            return d
        ...
        self.assertEqual(
            self.maybeStartBuildsOnBuilder_calls, ["A", "finished", "(stopped)"]
        )
...
class TestMaybeStartBuilds(TestBRDBase):
    ...
    @defer.inlineCallbacks
    def do_test_maybeStartBuildsOnBuilder(
        self, rows=None, exp_claims=None, exp_builds=None
    ):
        rows = rows or []
    ...
    def test_no_workerforbuilders(self):
        ...
        yield self.do_test_maybeStartBuildsOnBuilder(
            rows=rows, exp_claims=[], exp_builds=[]
        )
    ...
    def test_nextBuild_fails(self):
        ...
        result = self.do_test_maybeStartBuildsOnBuilder(
            rows=rows, exp_claims=[], exp_builds=[]
        )
    ...
    def do_test_nextWorker(
        self, nextWorker, global_select_next_worker, exp_choice=None
    ):
        ...
        self.bldr = yield self.createBuilder(
            "B", builderid=78, builder_config=builder_config
        )
        for i in range(4):
            self.addWorkers({f"test-worker{i}": 1})
