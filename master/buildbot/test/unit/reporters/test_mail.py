...
class TestMailNotifier(
    ...
    def test_createEmail_extraHeaders_two_builds(self):
        ...
        msgdict = create_msgdict()
        mn = yield self.setupMailNotifier(
            "from@example.org", extraHeaders={"hhh": "vvv"}
        )
    ...
    def test_createEmail_message_with_patch_and_log_containing_unicode(self):
        ...
        m = yield mn.createEmail(
            msgdict, "project-n\u00E5me", SUCCESS, [build], patches, logs
        )
    ...
    def setupBuildMessage(self, **generator_kwargs):
        ...
        generator = BuildStatusGenerator(
            message_formatter=formatter, **generator_kwargs
        )
    ...
    def do_test_sendToInterestedUsers(
        ...
        recipients = yield mn.findInterrestedUsersEmails(
            ["Big Bob <bob@mayhem.net>", "narrator"]
        )
        m = {"To": None, "CC": None}
    ...
    def do_test_sendMessage(self, **mn_kwargs):
        ...
        mn = yield self.setupMailNotifier(
            "from@example.org", generators=[generator], **mn_kwargs
        )
    ...
    def test_sendMessageOverTcp(self):
        ...
        self.assertIn(
            ("connectTCP", ("localhost", 25, None), {}), fakereactor.method_calls
        )
    ...
    def test_hostname_for_client_context_for_tcp_over_tls(self):
        ...
        self.assertEqual(
            "localhost", fakeSenderFactory.call_args.kwargs.get("hostname")
        )
    ...
    def test_hostname_for_client_context_for_plan_tcp(self):
        ...
        mn = yield self.setupMailNotifier(
            "John Doe <john.doe@domain.tld>", useTls=False
        )
        yield mn.sendMail(msg, ["Jane Doe <jane.doe@domain.tld>"])
