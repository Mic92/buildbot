    ...
    def makeScheduler(self, klass, **kwargs_override):
        ...
        def addBuildsetForChanges(
            ...
            **kw,
            ...
            self.events.append(
                f"B{repr(changeids).replace(' ', '')}@{int(self.clock.seconds())}"
            )
...
class BaseBasicScheduler(
    ...
    def test_activate_no_treeStableTimer(self):
        ...
        self.assertConsumingChanges(
            fileIsImportant=fII, change_filter=cf, onlyImportant=False
        )
    ...
    def test_activate_treeStableTimer(self):
        ...
        self.master.db.insert_test_data(
            [
                ...
                fakedb.SchedulerChange(
                    schedulerid=self.SCHEDULERID, changeid=20, important=1
                ),
            ]
        )
        ...
        # running
        self.assertConsumingChanges(
            fileIsImportant=None, change_filter=cf, onlyImportant=False
        )
    ...
    def test_gotChange_treeStableTimer_sequence(self):
        ...
        yield sched.gotChange(
            self.makeFakeChange(branch="master", number=1, when=2220), True
        )
        self.assertEqual(self.events, [])
        ...
        yield sched.gotChange(
            self.makeFakeChange(branch="master", number=2, when=2226), False
        )
        self.assertEqual(self.events, [])
        ...
        yield sched.gotChange(
            self.makeFakeChange(branch="master", number=3, when=2232), True
        )
        self.assertEqual(self.events, [])
...
class SingleBranchScheduler(
    ...
    def test_gotChange_createAbsoluteSourceStamps_saveCodebase(self):
        ...
        self.db.insert_test_data(
            [
                fakedb.Object(
                    id=self.OBJECTID, name="test", class_name="SingleBranchScheduler"
                )
            ]
        )
...
class AnyBranchScheduler(
    ...
    def test_gotChange_treeStableTimer_multiple_repositories(self):
        ...
        yield self.clock.advance(1)  # time is now 1
        yield sched.gotChange(
            mkch(branch="master", repository="repo", number=14), False
        )
        yield sched.gotChange(
            mkch(branch="master", repository="other_repo", number=15), False
        )
        yield self.clock.pump([1] * 4)  # time is now 5
        yield sched.gotChange(
            mkch(branch="master", repository="other_repo", number=17), True
        )
        yield self.clock.pump([1] * 10)  # time is now 15
    ...
    def test_gotChange_treeStableTimer_multiple_projects(self):
        ...
        yield sched.gotChange(
            mkch(branch="master", project="other_proj", number=15), False
        )
        yield self.clock.pump([1] * 4)  # time is now 5
        yield sched.gotChange(
            mkch(branch="master", project="other_proj", number=17), True
        )
        yield self.clock.pump([1] * 10)  # time is now 15
    ...
    def test_gotChange_treeStableTimer_multiple_codebases(self):
        ...
        yield sched.gotChange(
            mkch(branch="master", codebase="other_base", number=15), False
        )
        self.clock.pump([1] * 4)  # time is now 5
        yield sched.gotChange(
            mkch(branch="master", codebase="other_base", number=17), True
        )
