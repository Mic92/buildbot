...
class UpgradeMasterOptions(base.BasedirMixin, base.SubcommandOptions):
    ...
    optFlags = [
        ...
        [
            "develop",
            "d",
            "link to buildbot dir rather than copy, with no "
...
class CreateMasterOptions(base.BasedirMixin, base.SubcommandOptions):
    ...
    def postOptions(self):
        ...
        try:
            ...
            raise usage.UsageError(
                f"could not parse database URL '{self['db']}'"
            ) from e
...
class TryOptions(base.SubcommandOptions):
    ...
    def postOptions(self):
        ...
        if self["connect"] == "pb":
            if not self["master"]:
                raise usage.UsageError(
                    "master location must be specified" "for 'pb' connections"
                )
...
class UserOptions(base.SubcommandOptions):
    ...
    def postOptions(self):
        ...
        if not op:
            raise usage.UsageError(
                "you must specify an operation: add, " "remove, update, get"
            )
        ...
        if bb_username or bb_password:
            if op != "update":
                raise usage.UsageError(
                    "bb_username and bb_password only work " "with update"
                )
