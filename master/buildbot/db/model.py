...
class Model(base.DBConnectorComponent):
    ...
    buildrequests = sautils.Table(
        ...
        sa.Column(
            "priority", sa.Integer, nullable=False, server_default=sa.DefaultClause("0")
        ),
    ...
    buildsets = sautils.Table(
        ...
        sa.Column(
            ...
            sa.Integer,
            sa.ForeignKey(
                "builds.id", use_alter=True, name="parent_buildid", ondelete="SET NULL"
            ),
            nullable=True,
        ),
    ...
    changes = sautils.Table(
        ...
        sa.Column(
            "repository", sa.String(length=512), nullable=False, server_default=""
        ),
    ...
    builders = sautils.Table(
        ...
        sa.Column(
            ...
            sa.Integer,
            sa.ForeignKey(
                "projects.id", name="fk_builders_projectid", ondelete="SET NULL"
            ),
            nullable=True,
        ),
        # sha1 of name; used for a unique index
    ...
    sa.Index(
        "build_data_buildid_name", build_data.c.buildid, build_data.c.name, unique=True
    )
    ...
    sa.Index(
        "users_info_attrs", users_info.c.attr_type, users_info.c.attr_data, unique=True
    )
    ...
    sa.Index(
        "name_per_object", object_state.c.objectid, object_state.c.name, unique=True
    )
