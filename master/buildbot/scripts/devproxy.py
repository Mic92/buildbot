...
class DevProxy:
    ...
    def __init__(self, port, next_url, plugins, unsafe_ssl, auth_cookie):
        ...
        if auth_cookie:
            if (
                "TWISTED_SESSION" in auth_cookie
            ):  # user pasted the whole document.cookie part!
        ...
        self.session = aiohttp.ClientSession(
            connector=conn, trust_env=True, cookies=cookies
        )
        self.config = None
    ...
    def connection_error(self, error):
        return aiohttp.web.Response(
            text=f"Unable to connect to upstream server {self.next_url} "
            f"({error!s})",
            status=502,
        )
    ...
    async def fetch_config_from_upstream(self):
        ...
        for plugin in list(self.config["plugins"].keys()):
            if plugin not in self.apps:
                ...
                log.warn(
                    "warning: Missing plugin compared to original buildbot: %s", plugin
                )
