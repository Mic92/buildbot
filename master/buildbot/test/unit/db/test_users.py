...
class TestUsersConnectorComponent(
    ...
    user2_rows = [
        ...
        fakedb.UserInfo(
            uid=2, attr_type="git", attr_data="Tyler Durden <tyler@mayhem.net>"
        ),
    ...
    user3_rows = [
        fakedb.User(
            uid=3, identifier="marla", bb_username="marla", bb_password="cancer"
        )
    ]
    ...
    def test_addUser_existing_identifier(self):
        ...
        def thd(conn):
            ...
            users_info_tbl = self.db.model.users_info
            users = conn.execute(
                users_tbl.select(order_by=users_tbl.c.identifier)
            ).fetchall()
            infos = conn.execute(
                users_info_tbl.select(users_info_tbl.c.uid == uid)
            ).fetchall()
    ...
    def test_getUsers_multiple(self):
        ...
        self.assertEqual(
            res, [{"uid": 1, "identifier": "soap"}, {"uid": 2, "identifier": "lye"}]
        )
