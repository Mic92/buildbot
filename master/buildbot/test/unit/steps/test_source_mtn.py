...
class TestMonotone(
    ...
    def test_mode_full_clean_no_existing_db(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir=".", command=["mtn", "db", "init", "--db", "db.mtn"]
            ).exit(0),
            ExpectShell(
                workdir=".",
                command=[
    ...
    def test_mode_incremental_no_existing_db(self):
        self.setup_step(
            mtn.Monotone(
                repourl="mtn://localhost/monotone", mode="incremental", branch="master"
            )
        )
    ...
    def test_mode_full_no_method(self):
        self.setup_step(
            mtn.Monotone(
                repourl="mtn://localhost/monotone", mode="full", branch="master"
            )
        )
    ...
    def test_database_migration(self):
        ...
        self.expect_commands(
            ...
            .exit(0),
            ExpectShell(
                workdir=".", command=["mtn", "db", "migrate", "--db", "db.mtn"]
            ).exit(0),
            ExpectShell(
                workdir=".",
                command=[
