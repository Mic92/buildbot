...
class TransferCommand(Command):
    def finished(self, res):
        if self.debug:
            self.log_msg(
                "finished: stderr={0!r}, rc={1!r}".format(self.stderr, self.rc)
            )
...
class WorkerFileUploadCommand(TransferCommand):
    ...
    def do_protocol_write(self, data):
        return self.protocol_command.protocol_update_upload_file_write(
            self.writer, data
        )
...
class WorkerDirectoryUploadCommand(WorkerFileUploadCommand):
    ...
    def start(self):
        ...
        with tarfile.open(mode=mode, fileobj=self.fp) as archive:
            try:
                ...
                self.stderr = "Cannot read directory '{0}' for upload: {1}".format(
                    self.path, e
                )
                self.rc = 1
    ...
    def do_protocol_write(self, data):
        return self.protocol_command.protocol_update_upload_directory_write(
            self.writer, data
        )
...
class WorkerFileDownloadCommand(TransferCommand):
    ...
    def _writeData(self, data):
        if self.debug:
            self.log_msg(
                "WorkerFileDownloadCommand._readBlock(): readlen={0}".format(len(data))
            )
        if not data:
            return True
