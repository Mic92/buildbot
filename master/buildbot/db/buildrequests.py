...
class BuildRequestsConnectorComponent(base.DBConnectorComponent):
    def _saSelectQuery(self):
        ...
        from_clause = from_clause.join(
            bsets_tbl, reqs_tbl.c.buildsetid == bsets_tbl.c.id
        )
        from_clause = from_clause.join(
        ...
        from_clause = from_clause.join(
            builder_tbl, reqs_tbl.c.builderid == builder_tbl.c.id
        )
    ...
    def getBuildRequests(
        ...
        def thd(conn):
            ...
            return deduplicateBrdict(
                [
                    self._brdictFromRow(row, self.db.master.masterid)
    ...
    def completeBuildRequests(self, brids, results, complete_at=None):
        assert (
            results != RETRY
        ), "a buildrequest cannot be completed with a retry status!"
        ...
        def thd(conn):
            ...
            for batch in self.doBatch(brids, 100):
                ...
                res = conn.execute(
                    q, complete=1, results=results, complete_at=complete_at
                )
