...
class TestForceScheduler(
    ...
    def test_compare_username(self):
        self.assertNotEqual(
            ...
            ForceScheduler(
                name="testched",
                builderNames=[],
                username=FixedParameter(
                    "username", "The Fisher King <avallach@atlantis.al>"
                ),
            ),
        )
    ...
    def test_bad_codebases(self):
        ...
        with self.assertRaisesConfigError(
            ...
            ForceScheduler(
                name="foo", builderNames=["bar"], codebases=[IntParameter("foo")]
            )
        ...
        with self.assertRaisesConfigError(errMsg):
            ForceScheduler(
                name="foo", builderNames=["bar"], codebases={"cb": {"branch": "trunk"}}
            )
    ...
    def test_BooleanParameter_False(self):
        ...
        self.do_ParameterTest(
            value="123", expect=False, klass=BooleanParameter, req=req
        )
    ...
    def test_bad_reason(self):
        with self.assertRaisesConfigError(
            ...
            ForceScheduler(
                name="testsched", builderNames=[], codebases=["bar"], reason="foo"
            )
    ...
    def test_bad_username(self):
        with self.assertRaisesConfigError(
            ...
            ForceScheduler(
                name="testsched", builderNames=[], codebases=["bar"], username="foo"
            )
    ...
    def test_notstring_name(self):
        with self.assertRaisesConfigError(
            ...
            ForceScheduler(
                name=1234, builderNames=[], codebases=["bar"], username="foo"
            )
    ...
    def test_notidentifier_name(self):
        ...
        with self.assertRaisesConfigError(
            ...
            ForceScheduler(
                name="my scheduler", builderNames=[], codebases=["bar"], username="foo"
            )
