...
class TestResultSubmitter:
    ...
    def _process_batches(self):
        ...
        while self._pending_batches:
            ...
            yield self._master.data.updates.addTestResults(
                self._builderid, self._setid, batch
            )
    ...
    def _initialize_pass_fail_recording_if_needed(self):
        if self._category == "pass_fail" and self._value_unit == "boolean":
            self._initialize_pass_fail_recording(
                self._add_pass_fail_result_category_pass_fail
            )
            return
        if self._category == "pass_only":
            self._initialize_pass_fail_recording(
                self._add_pass_fail_result_category_pass_only
            )
            return
        if self._category in ("fail_only", "code_issue"):
            self._initialize_pass_fail_recording(
                self._add_pass_fail_result_category_fail_only
            )
            return
