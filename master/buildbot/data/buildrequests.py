...
class Db2DataMixin:
    ...
    def addPropertiesToBuildRequest(self, buildrequest, filters):
        if not filters:
            return None
        props = yield self.master.db.buildsets.getBuildsetProperties(
        ...
        try:
            ...
            for b in builds:
                self.master.mq.produce(
                    ("control", "builds", str(b["buildid"]), "stop"), mqArgs
                )
            return None
...
class BuildRequestsEndpoint(Db2DataMixin, base.Endpoint):
    ...
    def get(self, resultSpec, kwargs):
        ...
        results = []
        filters = (
            resultSpec.popProperties() if hasattr(resultSpec, "popProperties") else []
        )
