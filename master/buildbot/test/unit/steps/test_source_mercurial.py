...
class TestMercurial(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_mode_full_clean(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["hg", "--verbose", "--version"]).exit(
                0
            ),
            ...
            ExpectShell(
                workdir="wkdir", command=["hg", "--verbose", "identify", "--branch"]
            )
            .stdout("default")
            .exit(0),
            ExpectShell(
    ...
    def test_mode_full_clean_timeout(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", timeout=1, command=["hg", "--verbose", "--version"]
            ).exit(0),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ExpectStat(file="wkdir/.hg", log_environ=True).exit(0),
            ExpectShell(
    ...
    def test_mode_incremental_no_existing_repo_dirname(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="wkdir", command=["hg", "--verbose", "update", "--clean"]
            ).exit(0),
            ExpectShell(
                workdir="wkdir",
                command=["hg", "--verbose", "parents", "--template", "{node}\\n"],
    ...
    def test_worker_connection_lost(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", command=["hg", "--verbose", "--version"]
