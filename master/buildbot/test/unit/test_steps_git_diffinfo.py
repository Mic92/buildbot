...
class TestDiffInfo(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_merge_base_failure(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", command=["git", "merge-base", "HEAD", "master"]
            )
            .log("stdio-merge-base", stderr="fatal: Not a valid object name")
            .exit(128)
        )
        self.expect_log_file_stderr(
            "stdio-merge-base", "fatal: Not a valid object name\n"
        )
        self.expect_outcome(
            result=results.FAILURE, state_string="GitDiffInfo (failure)"
        )
