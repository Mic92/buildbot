...
class VersioningUtilsTests(unittest.SynchronousTestCase):
    ...
    def setUp(self):
        try:
            ...
            raise unittest.SkipTest(
                self.module_under_test + " package is not installed"
            ) from e

    def test_gitDescribeToPep440devVersion(self):
        self.assertEqual(
            self.m.gitDescribeToPep440("v0.9.8-20-gf0f45ca"), "0.9.9-dev20"
        )
    ...
    def test_gitDescribeToPep440p1dev(self):
        self.assertEqual(
            self.m.gitDescribeToPep440("v0.9.9.post1-20-gf0f45ca"), "0.9.10-dev20"
        )
    ...
    def test_getVersionFromArchiveIdtag(self):
        version = self.m.getVersionFromArchiveId(
            "1514808197  (HEAD -> master, tag: v1.0.0)"
        )
