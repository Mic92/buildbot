...
class TicketLoginProtocol(protocol.ProcessProtocol):
    ...
    def processEnded(self, reason):
        if debug_logging:
            log.msg(
                f"P4Poller: login process finished for {self.p4base}: {reason.value.exitCode}"
            )
...
class P4Source(base.ReconfigurablePollingChangeSource, util.ComparableMixin):
    ...
    def checkConfig(
        ...
        if server_tz is not None and dateutil.tz.gettz(server_tz) is None:
            raise P4PollerError(
                f"Failed to get timezone from server_tz string '{server_tz}'"
            )
    ...
    def _poll(self):
        ...
        for num in changelists:
            ...
            try:
                ...
                log.msg(
                    f"P4Poller: couldn't decode changelist description: {ex.encoding}"
                )
            ...
            if not m:
                raise P4PollerError(
                    f"Unexpected 'p4 describe -s' result: {repr(result)}"
                )
