...
class Test(TestReactorMixin, www.WwwTestMixin, unittest.TestCase):
    ...
    def test_reconfigService_expiration_time(self):
        ...
        self.assertNotEqual(self.svc.port_service, None)
        self.assertEqual(
            service.BuildbotSession.expDelay, datetime.timedelta(minutes=1)
        )
    ...
    def test_setupSite(self):
        ...
        self.assertIsInstance(
            root.getChildWithDefault(b"api", req), rest.RestRootResource
        )
    ...
    def test_setupSiteWithProtectedHook(self):
        ...
        self.svc.setupSite(
            self.makeConfig(
                change_hook_dialects={"base": True}, change_hook_auth=[checker]
            )
        )
        site = self.svc.site
...
class TestBuildbotSite(unittest.SynchronousTestCase):
    ...
    def test_getSession_from_correct_jwt(self):
        ...
        uid = jwt.encode(
            payload, self.SECRET, algorithm=service.SESSION_SECRET_ALGORITHM
        )
    ...
    def test_updateSession(self):
        ...
        decoded = jwt.decode(
            value, self.SECRET, algorithms=[service.SESSION_SECRET_ALGORITHM]
        )
