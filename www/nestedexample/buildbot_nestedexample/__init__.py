...
class NestedExample(NestedParameter):
    ...
    def validateProperties(self, collector, properties):
        ...
        # the ingredients
        if (
            properties[self.INGREDIENTS] not in self.allIngredients
            or not properties[self.PIZZA]
        ):
    ...
    def updateFromKwargs(self, kwargs, properties, collector, **kw):
        yield super(NestedExample, self).updateFromKwargs(
            kwargs, properties, collector, **kw
        )
