...
class BaseScheduler(scheduler.SchedulerMixin, TestReactorMixin, unittest.TestCase):
    ...
    def makeScheduler(
        self, name="testsched", builderNames=None, properties=None, codebases=None
    ):
    ...
    def test_change_consumption_refs_heads_branch_deprecated(
        self, name, cf, expected_result
    ):
    ...
    def test_addBuildsetForChanges_codebases_set_multiple_codebases(self):
        ...
        sched = self.makeScheduler(
            name="n", builderNames=["b", "c"], codebases=codebases
        )
        self.db.insert_test_data(
            [
