...
class BuildsConnectorComponent(base.DBConnectorComponent):
    ...
    def getBuildsForChange(self, changeid):
        ...
        def thd(conn):
            ...
            from_clause = from_clause.join(
                bsets_tbl, bsss_tbl.c.buildsetid == bsets_tbl.c.id
            )
            from_clause = from_clause.join(
                reqs_tbl, bsets_tbl.c.id == reqs_tbl.c.buildsetid
            )
            from_clause = from_clause.join(
        ...
        def thd(conn):
            ...
            while True:
                ...
                try:
                    ...
                    # numbers
                    if (
                        'duplicate key value violates unique constraint "builds_pkey"'
                        not in str(e)
                    ):
                        new_number += 1
                    continue
    ...
    def finishBuild(self, buildid, results):
        def thd(conn):
            ...
            q = tbl.update(whereclause=tbl.c.id == buildid)
            conn.execute(
                q, complete_at=int(self.master.reactor.seconds()), results=results
            )
