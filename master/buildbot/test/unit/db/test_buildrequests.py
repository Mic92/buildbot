...
class Tests(interfaces.InterfaceTests):
    ...
    def do_test_getBuildRequests_claim_args(self, **kwargs):
        ...
        self.assertEqual(
            sorted([br["buildrequestid"] for br in brlist]), sorted(expected)
        )
    ...
    def test_getBuildRequests_claimed_mine(self):
        return self.do_test_getBuildRequests_claim_args(
            claimed=self.MASTER_ID, expected=[50]
        )
    ...
    def test_getBuildRequests_complete_true(self):
        return self.do_test_getBuildRequests_complete_arg(
            complete=True, expected=[80, 81]
        )

    def test_getBuildRequests_complete_false(self):
        return self.do_test_getBuildRequests_complete_arg(
            complete=False, expected=[70, 82]
        )
    ...
    def test_getBuildRequests_bsid_arg(self):
        ...
        self.assertEqual(
            sorted([br["buildrequestid"] for br in brlist]), sorted([70, 72])
        )
        ...
        yield self.insert_test_data(
            [
                ...
                fakedb.BuildsetSourceStamp(
                    buildsetid=self.BSID + 1, sourcestampid=self.BSID + 1
                ),
                ...
                fakedb.BuildsetSourceStamp(
                    buildsetid=self.BSID + 2, sourcestampid=self.BSID + 2
                ),
                ...
                fakedb.SourceStamp(
                    id=self.BSID + 3, branch="branch_A", repository="repository_A"
                ),
                fakedb.BuildsetSourceStamp(
                    buildsetid=self.BSID + 3, sourcestampid=self.BSID + 3
                ),
                # multiple sourcestamps on the same buildset are possible
                fakedb.SourceStamp(
                    id=self.BSID + 4, branch="branch_B", repository="repository_B"
                ),
                fakedb.BuildsetSourceStamp(
                    buildsetid=self.BSID + 3, sourcestampid=self.BSID + 4
                ),
            ]
        )
        brlist = yield self.db.buildrequests.getBuildRequests(**kwargs)
    ...
    def test_getBuildRequests_branch(self):
        return self.do_test_getBuildRequests_branch_arg(
            branch="branch_A", expected=[70, 90]
        )

    def test_getBuildRequests_branch_empty(self):
        return self.do_test_getBuildRequests_branch_arg(
            branch="absent_branch", expected=[]
        )
    ...
    def do_test_claimBuildRequests(
        ...
        try:
            ...
            yield self.db.buildrequests.claimBuildRequests(
                brids=brids, claimed_at=claimed_at
            )
            results = yield self.db.buildrequests.getBuildRequests()

            self.assertNotEqual(
                expected, None, "unexpected success from claimBuildRequests"
            )
            self.assertEqual(
                sorted(
                    [
    ...
    def do_test_completeBuildRequests(
        ...
        try:
            ...
            self.assertNotEqual(
                expected, None, "unexpected success from completeBuildRequests"
            )
            self.assertEqual(
                sorted(
    ...
    def do_test_unclaimMethod(self, method, expected):
        ...
        self.assertEqual(
            sorted([r["buildrequestid"] for r in results]), sorted(expected)
        )
...
class TestFakeDB(
    unittest.TestCase, connector_component.FakeConnectorComponentMixin, Tests
):
