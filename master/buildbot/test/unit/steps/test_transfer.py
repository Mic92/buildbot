...
class TestFileUpload(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def testTimestamp(self):
        self.setup_step(
            transfer.FileUpload(
                workersrc=__file__, masterdest=self.destfile, keepstamp=True
            )
        )
        ...
        self.expect_outcome(
            result=SUCCESS, state_string=f"uploading {os.path.basename(__file__)}"
        )
    ...
    def test_interrupt(self):
        ...
        self.expect_outcome(
            result=CANCELLED, state_string="uploading srcfile (cancelled)"
        )
...
class TestDirectoryUpload(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def testException(self):
        ...
        self.expect_outcome(
            result=EXCEPTION, state_string="uploading srcdir (exception)"
        )
        yield self.run_step()
...
class TestFileDownload(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_no_file(self):
        self.setup_step(
            transfer.FileDownload(
                mastersrc="not existing file", workerdest=self.destfile
            )
        )
        ...
        self.expect_log_file(
            "stderr", "File 'not existing file' not available at master"
        )
        yield self.run_step()
...
class TestJSONPropertiesDownload(
