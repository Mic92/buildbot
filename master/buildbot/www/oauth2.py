...
class GitHubAuth(OAuth2Auth):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if apiVersion == 3:
            ...
            self.apiResourceEndpoint = (
                self.apiResourceEndpoint or defaultGraphqlEndpoint
            )
    ...
    def post(self, session, query):
        if self.debug:
            log.info(
                "{klass} GraphQL POST Request: {endpoint} -> "
                "DATA:\n----\n{data}\n----",
    ...
    def getUserInfoFromOAuthClient_v4(self, c):
        ...
        user_info = {
            ...
            "groups": [
                org["node"]["login"] for org in data["viewer"]["organizations"]["edges"]
            ],
        }
            ...
            if graphql_query:
                ...
                for org, team_data in data["data"].items():
                    ...
                    for node in team_data["teams"]["edges"]:
                        ...
                        # team name
                        teams.add(
                            f"{orgs_name_slug_mapping[org]}/{node['node']['name']}"
                        )
                        teams.add(
                            f"{orgs_name_slug_mapping[org]}/{node['node']['slug']}"
                        )
