...
class BBRefTargetDirective(Directive):
    ...
    @classmethod
    def resolve_ref(
        cls, domain, env, fromdocname, builder, typ, target, node, contnode
    ):
        """
        ...
        return make_refnode(
            builder, fromdocname, todocname, targetname, contnode, target
        )
...
class BBIndex(Index):
    ...
    def generate(self, docnames=None):
        ...
        content = [
            (l, sorted(content[l], key=lambda tup: tup[0].lower()))
        ...
        for typ in self.object_types:
            ...
            for target_name, target_data in other_data.items():
                if target_name in self_data:
                    ...
                    logger.warning(
                        (
                            "Duplicate index {} reference {} in {}, "
                            "other instance in {}"
                        ).format(typ, target_name, self_path, other_path)
                    )
                else:
