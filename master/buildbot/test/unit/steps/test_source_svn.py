...
class TestSVN(sourcesteps.SourceStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_svn_not_installed(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["svn", "--version"]).exit(1)
        )
    ...
    def test_mode_incremental_timeout(self):
        ...
        self.expect_commands(
            ExpectShell(workdir="wkdir", timeout=1, command=["svn", "--version"]).exit(
                0
            ),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ExpectStat(file="wkdir/.svn", log_environ=True).exit(0),
    ...
    def test_mode_incremental_repourl_renderable(self):
        self.setup_step(
            svn.SVN(
                repourl=ConstantRenderable("http://svn.local/trunk"), mode="incremental"
            )
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["svn", "--version"]).exit(0),
            ...
            .stdout(
                """<?xml version="1.0"?>""" + """<url>http://svn.local/trunk</url>"""
            )
            .exit(0),
            ExpectShell(
                workdir="wkdir",
    ...
    def test_mode_incremental_repourl_canonical(self):
        self.setup_step(
            svn.SVN(repourl="http://svn.local/trunk/test app", mode="incremental")
        )
        self.expect_commands(
            ExpectShell(workdir="wkdir", command=["svn", "--version"]).exit(0),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
    ...
    def test_mode_full_clobber_given_revision(self):
        self.setup_step(
            svn.SVN(
                repourl="http://svn.local/app/trunk", mode="full", method="clobber"
            ),
            {"revision": "100"},
        )
        self.expect_commands(
    ...
    def test_mode_full_export(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(workdir="", command=["svn", "export", "source", "wkdir"]).exit(
                0
            ),
    ...
    def test_mode_full_export_timeout(self):
        ...
        self.expect_commands(
            ...
            ExpectShell(
                workdir="", timeout=1, command=["svn", "export", "source", "wkdir"]
            ).exit(0),
    ...
    def test_mode_incremental_with_env(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", command=["svn", "--version"], env={"abc": "123"}
            ).exit(0),
            ExpectStat(file="wkdir/.buildbot-patched", log_environ=True).exit(1),
            ExpectStat(file="wkdir/.svn", log_environ=True).exit(0),
            ExpectShell(
            ...
            ExpectShell(
                workdir="wkdir", command=["svn", "info", "--xml"], env={"abc": "123"}
            )
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
    ...
    def test_mode_incremental_log_environ(self):
        ...
        self.expect_commands(
            ExpectShell(
                workdir="wkdir", command=["svn", "--version"], log_environ=False
            ).exit(0),
            ...
            ExpectShell(
                workdir="wkdir", command=["svn", "info", "--xml"], log_environ=False
            )
            .stdout(self.svn_info_stdout_xml)
            .exit(0),
        )
