    ...
    def collapse(self):
        ...
        for brid in self.brids:
            ...
            for unclaim_br in unclaim_brs:
                ...
                canCollapse = yield collapseRequestsFn(
                    self.master, bldr, br, unclaim_br
                )
...
class BuildRequest:
    ...
    def _make_br(cls, brid, brdict, master):
        ...
        buildset_properties = yield master.db.buildsets.getBuildsetProperties(
            brdict["buildsetid"]
        )
    ...
    def canBeCollapsed(master, new_br, old_br):
        ...
        otherSources = dict(
            (ss["codebase"], ss) for ss in otherBuildsets["sourcestamps"]
        )
        ...
        for c, selfSS in selfSources.items():
            ...
            if selfSS["patch"] or otherSS["patch"]:
                return False
            # get changes & compare
            selfChanges = yield master.data.get(
                ("sourcestamps", selfSS["ssid"], "changes")
            )
            otherChanges = yield master.data.get(
                ("sourcestamps", otherSS["ssid"], "changes")
            )
