...
class WorkerManager(MeasuredBuildbotServiceManager):
    ...
    def newConnection(self, conn, workerName):
        if workerName in self.connections:
            log.msg(
                f"Got duplication connection from '{workerName}'"
            ...
            try:
                ...
                old_conn.loseConnection()
                log.msg(
                    f"Got error while trying to ping connected worker {workerName}:{e}"
                )
        ...
        try:
            ...
            log.msg(
                f"Failed to communicate with worker '{workerName}'\n{e}".format(
                    workerName, e
                )
            )
            raise
