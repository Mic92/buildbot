...
class TestRunProcess(BasedirMixin, unittest.TestCase):
    ...
    def testCommandEncodingList(self):
        s = runprocess.RunProcess(
            0, ["abcd", b"efg"], self.basedir, "utf-8", self.send_update
        )
    ...
    def testStringCommand(self):
        ...
        s = runprocess.RunProcess(
            0, "echo hello", self.basedir, "utf-8", self.send_update
        )
    ...
    def testMultiWordStringCommandQuotes(self):
        ...
        s = runprocess.RunProcess(
            0,
            'echo "Happy Days and Jubilation"',
            self.basedir,
            "utf-8",
            self.send_update,
        )
    ...
    def testCommandTimeout(self):
        ...
        self.assertTrue(
            ("failure_reason", "timeout_without_output") in self.updates, self.show()
        )
    ...
    def test_startCommand_exception(self):
        s = runprocess.RunProcess(
            0, ["whatever"], self.basedir, "utf-8", self.send_update
        )
        ...
        stderr = "".join(stderr)
        self.assertTrue(
            stderr.startswith("error in RunProcess._startCommand (error message)")
        )
    ...
    def testEnvironArray(self):
        ...
        self.assertFalse(
            re.match("\bFOO=a{0}b\b".format(os.pathsep), headers), "got:\n" + headers
        )
...
class TestPOSIXKilling(BasedirMixin, unittest.TestCase):
    ...
    def test_pgroup_no_usePTY_no_pgroup(self):
        ...
        return self.do_test_pgroup(
            usePTY=False, useProcGroup=False, expectChildSurvival=True
        )
    ...
    def test_double_fork_no_usePTY_no_pgroup(self):
        # note that this configuration is not *used*, but that it is
        # still supported, and correctly fails to kill the child process
        return self.do_test_double_fork(
            usePTY=False, useProcGroup=False, expectChildSurvival=True
        )
...
class TestLogFileWatcher(BasedirMixin, unittest.TestCase):
    ...
    def test_invalid_utf8(self):
        ...
        try:
            lf = runprocess.LogFileWatcher(
                rp, "test", test_filename, follow=False, poll=False
            )
