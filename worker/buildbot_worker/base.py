...
class ProtocolCommandBase:
    def __init__(
        ...
        try:
            ...
            raise UnknownCommand(
                "(command {0}): unrecognized WorkerCommand '{1}'".format(
                    command_id, command
                )
            )
    ...
    def command_complete(self, failure):
        if failure:
            self.log_msg(
                "ProtocolCommandBase.command_complete (failure) {0}".format(
                    self.command
                )
            )
            log.err(failure)
            ...
            # failure is None
            self.log_msg(
                "ProtocolCommandBase.command_complete (success) {0}".format(
                    self.command
                )
            )
...
class BotBase(service.MultiService):
    ...
    def __init__(self, basedir, unicode_encoding=None, delete_leftover_dirs=False):
        ...
        self.unicode_encoding = (
            unicode_encoding or sys.getfilesystemencoding() or "ascii"
        )
