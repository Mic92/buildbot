            ...
            try:
                ...
                self.sendStatus(
                    [
                        (
                            "header",
                            "{0}: {1}: {2}".format(self.header, e.strerror, dirname),
                        ),
                        ("rc", e.errno),
                    ]
                )
    ...
    def start(self):
        ...
        if runtime.platformType != "posix":
            ...
            if os.path.exists(to_path):
                ...
                self.log_msg(
                    (
                        "cp target '{0}' already exists -- cp will not do what you think!"
                    ).format(to_path)
                )
        ...
        try:
            ...
            self.sendStatus(
                [
                    (
                        "header",
                        "{0}: {1}: {2}".format(self.header, e.strerror, filename),
                    ),
                    ("rc", e.errno),
                ]
            )
        ...
        try:
            ...
            self.sendStatus(
                [
                    (
                        "header",
                        "{0}: {1}: {2}".format(self.header, e.strerror, pathname),
                    ),
                    ("rc", e.errno),
                ]
            )
