...
class LogChunkEndpointBase(endpoint.EndpointMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db.insert_test_data(
            [
                ...
                fakedb.Log(
                    id=60, stepid=50, name="stdio", slug="stdio", type="s", num_lines=7
                ),
                fakedb.LogChunk(
                    logid=60,
                    first_line=0,
            ...
            + [
                fakedb.Log(
                    id=62, stepid=50, name="notes", slug="notes", type="t", num_lines=0
                ),
                # logid 62 is empty
            ]
        )
    ...
    def do_test_chunks(self, path, logid, expLines):
        ...
        expContent = "\n".join(expLines) + "\n"
        self.assertEqual(
            logchunk, {"logid": logid, "firstline": 0, "content": expContent}
        )
        ...
        for i, expLine in enumerate(expLines):
            ...
            self.validateData(logchunk)
            self.assertEqual(
                logchunk, {"logid": logid, "firstline": i, "content": expLine + "\n"}
            )
        ...
        for f, length in (0, mid), (mid, len(expLines) - 1):
            ...
            self.assertEqual(
                logchunk, {"logid": logid, "firstline": f, "content": expContent}
            )
        ...
        self.assertEqual(
            (
                yield self.callGet(
                    path, resultSpec=resultspec.ResultSpec(offset=10, limit=-1)
                )
            ),
            None,
        )
